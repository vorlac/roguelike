...
namespace rl::gl {
    ...
    using enum_t = u32;
    using bool_t = u8;
    using bitfield_t = u32;
    using void_t = void;
    using byte_t = i8;
    // OpenGL 1-byte signed
    using short_t = i16;   // OpenGL 2-byte signed
    using int_t = i32;     // OpenGL 4-byte signed
    using ubyte_t = u8;    // OpenGL 1-byte unsigned
    using ushort_t = u16;  // OpenGL 2-byte unsigned
    using uint_t = u32;    // OpenGL 4-byte unsigned
    using sizei_t = i32;   // OpenGL 4-byte signed
    using float_t = f32;   // OpenGL single precision float
    using clampf_t = f32;  // OpenGL single precision float in [0,1]
    using double_t = f64;  // OpenGL double precision float
    using clampd_t = f64;  // OpenGL double precision float in [0,1]

    // OpenGL Boolean
    enum glbool : gl::bool_t {
        False = GL_FALSE,
        True = GL_TRUE,
    };

    // OpenGL Data Type
    enum gltype : gl::enum_t {
        SByte = GL_BYTE,             // signed - byte
        UByte = GL_UNSIGNED_BYTE,    // unsigned - byte
        SShort = GL_SHORT,           // signed short - 2 bytes
        UShort = GL_UNSIGNED_SHORT,  // unsigned - 2 bytes
        SInt = GL_INT,               // signed - 4 bytes
        UInt = GL_UNSIGNED_INT,      // unsigned - 4 bytes

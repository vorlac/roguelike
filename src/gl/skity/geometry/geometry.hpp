...
namespace skity {
    ...
    static inline glm::vec2 FromPoint(const Point& p)
    {
        return glm::vec2{ p.x, p.y };
    }

    static inline Point ToPoint(const glm::vec2& x)
    {
    ...
    struct QuadCoeff
    {
        ...
        QuadCoeff(const glm::vec2& a, const glm::vec2& b, const glm::vec2& c)
            : A{ a }
            , B{ b }
            , C{ c }
        {
        ...
        explicit QuadCoeff(const std::array<Point, 3>& src);

        explicit QuadCoeff(const std::array<glm::vec2, 3>& src);

        Point evalAt(float t);

        glm::vec2 eval(float t);

        glm::vec2 eval(const glm::vec2& tt);

        static Point EvalQuadAt(const std::array<Point, 3>& src, float t);

        static void EvalQuadAt(const std::array<Point, 3>& src, float t, Point* outP,
                               Vector* outTangent);

        static Vector EvalQuadTangentAt(const std::array<Point, 3>& src, float t);

        static glm::vec2 EvalQuadTangentAt(const glm::vec2& p1, const glm::vec2& p2,
    ...
    struct CubicCoeff
    {
        ...
        explicit CubicCoeff(const std::array<Point, 4>& src);

        Point evalAt(float t);

        glm::vec2 eval(float t);

        glm::vec2 eval(const glm::vec2& t);

        static void EvalCubicAt(const Point src[4], float t, Point* loc, Vector* tangent,
                                Vector* curvature);
    ...
    */
    static inline void formulate_F1DotF2(const float src[], float coeff[4])
    {
        float a = src[2] - src[0];

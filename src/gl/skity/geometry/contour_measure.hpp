...
namespace skity {
    ...
    public:
        ...
        bool getSegment(float startD, float stopD, Path* dst, bool startWithMoveTo) const;
        ...
        struct Segment
        {
            ...
            const static Segment* Next(const Segment* seg)
            {
        ...
        ContourMeasure(std::vector<Segment>&& segs, std::vector<Point>&& pts, float length,
                       bool isClosed);
    ...
    public:
         ...
         */
        ContourMeasureIter(const Path& path, bool forceClosed, float resScale = 1.f);

        ~ContourMeasureIter();

        void reset(const Path&, bool forceClosed, float resScale = 1.f);

...
namespace skity {
    ...
    {
        return FloatHalf * (right_ + left_);
    }

    float Rect::centerY() const
    {
        return FloatHalf * (top_ + bottom_);
    }

    Rect Rect::makeSorted() const
    {
        return MakeLTRB(std::min(left_, right_), std::min(top_, bottom_), std::max(left_, right_),
    ...
    float Rect::HalfWidth(const Rect& rect)
    {
        return rect.right_ * FloatHalf - rect.left_ * FloatHalf;
    }

    float Rect::HalfHeight(const Rect& rect)
    {

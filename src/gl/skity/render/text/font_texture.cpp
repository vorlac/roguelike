...
#include "gl/skity/render/text/font_texture.hpp"

namespace skity {

    FontTexture::FontTexture(Typeface* typeface)
        : TextureAtlas(DEFAULT_SIZE, DEFAULT_SIZE, 1)
        , typeface_(typeface)
    {
    }
    ...
    glm::ivec4 FontTexture::GenerateGlyphRegion(const GlyphKey& key)
    {
        ...
        // upload text bitmap
        UploadRegion(region.x, region.y, bitmap_info.width, bitmap_info.height, bitmap_info.buffer);

...
namespace skity {
    ...
    std::shared_ptr<Data> MakeCopyWithString(const std::string& str)
    {
        if (str.empty())
            return nullptr;
                                             ...
                                             const std::vector<uint32_t>& colors)
    {
        ...
        return Shader::MakeRadial(center, radius, f_colors.data(), nullptr, f_colors.size());
    }
                                             ...
                                             const std::vector<uint32_t>& colors)
    {
        ...
        return Shader::MakeLinear(pts.data(), f_colors.data(), nullptr, f_colors.size());
    }

    std::shared_ptr<Shader> MakeLinearShaderWithPos(float x1, float y1, float x2, float y2,
                                                    const std::vector<uint32_t>& colors,
                                                    const std::vector<float>& pos)
    {
        ...
        return Shader::MakeLinear(pts.data(), f_colors.data(), pos.data(), f_colors.size());
    }

    std::shared_ptr<PathEffect> MakeDashEffect(const std::vector<float>& pattern)
    {
        return PathEffect::MakeDashPathEffect(pattern.data(), pattern.size(), 0);
    }

    glm::mat4 MatrixTranslate(const glm::mat4& m, float x, float y)
    {
        return glm::translate(m, glm::vec3(x, y, 0.f));
    }

    glm::mat4 MatrixRotate(const glm::mat4& m, float angle, float x, float y, float z)
    {
        return glm::rotate(m, angle, glm::vec3(x, y, z));
    }

    glm::mat4 MatrixMultiply(const glm::mat4& m1, const glm::mat4& m2)
    {
        return m1 * m2;
    }
...
//         .class_function("MakeDiscretePathEffect", &skity::PathEffect::MakeDiscretePathEffect)
//         .class_function("MakeDashEffect", &skity::MakeDashEffect);
//
//     class_<skity::TextBlob>("TextBlob").smart_ptr<std::shared_ptr<skity::TextBlob>>("TextBlob");
//
//     class_<skity::TextBlobBuilder>("TextBlobBuilder")
//         .constructor()
//         .function("buildTextBlob", select_overload<std::shared_ptr<skity::TextBlob>(
//                                        const std::string&, const skity::Paint&)>(
...
//         .function("quadTo",
//                   select_overload<skity::Path&(float, float, float,
//                   float)>(&skity::Path::quadTo))
...
//                   select_overload<void(const std::shared_ptr<skity::TextBlob>&, float, float,
//                                        const skity::Paint&)>(&skity::Canvas::drawTextBlob))

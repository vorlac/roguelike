...
namespace skity {
    ...
    std::unique_ptr<Canvas> Canvas::MakeWebGLCanvas(const std::string& name, uint32_t width,
                                                    uint32_t height, float density)
    {
        ...
        EMSCRIPTEN_WEBGL_CONTEXT_HANDLE context = emscripten_webgl_create_context(name.c_str(),
                                                                                  &attrs);
        ...
        GPUContext ctx(GPUBackendType::kWebGL2, (void*)emscripten_webgl_get_proc_address);

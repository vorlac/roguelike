...
namespace skity {
    ...
    public:
        ...
        const std::shared_ptr<Typeface>& getDefaultTypeface() const
        {
         ...
         */
        void drawSimpleText(const char* text, float x, float y, const Paint& paint);
         ...
         *
         */
        void drawSimpleText2(const char* text, float x, float y, const Paint& paint);

        Vec2 simpleTextBounds(const char* text, const Paint& paint);

        void drawTextBlob(const TextBlob* blob, float x, float y, const Paint& paint);

        void drawTextBlob(const std::shared_ptr<TextBlob>& blob, float x, float y,
                          const Paint& paint)
        {
            drawTextBlob(blob.get(), x, y, paint);
        }

        inline void drawDebugLine(bool debug)
        {
            draw_debug_line_ = debug;
        }
         ...
         */
        static std::unique_ptr<Canvas> MakeHardwareAccelationCanvas(uint32_t width, uint32_t height,
                                                                    float density, GPUContext* ctx);

#ifdef SKITY_WASM
        static std::unique_ptr<Canvas> MakeWebGLCanvas(const std::string& name, uint32_t width,
                                                       uint32_t height, float density);
#endif
    ...
    protected:
        ...
        virtual void onClipRect(const Rect& rect, ClipOp op);
        virtual void onClipPath(const Path& path, ClipOp op) = 0;

        // default implement dispatch this to onDrawPath
        virtual void onDrawLine(float x0, float y0, float x1, float y1, const Paint& paint);
        // default implement dispatch this to onDrawPath
        virtual void onDrawCircle(float cx, float cy, float radius, const Paint& paint);
        // default implement dispatch this to onDrawPath
        virtual void onDrawOval(const Rect& oval, const Paint& paint);
        // default implement dispatch this to onDrawPath
        virtual void onDrawRect(const Rect& rect, const Paint& paint);
        // default implement dispatch this to onDrawPath
        virtual void onDrawRRect(const RRect& rrect, const Paint& paint);
        // default implement dispatch this to onDrawPath
        virtual void onDrawRoundRect(const Rect& rect, float rx, float ry, const Paint& paint);

        virtual void onDrawPath(const Path& path, const Paint& paint) = 0;

        virtual void onDrawBlob(const TextBlob* blob, float x, float y, const Paint& paint) = 0;
        ...
        virtual void onConcat(const Matrix& matrix) = 0;
        virtual void onFlush() = 0;
        virtual uint32_t onGetWidth() const = 0;
        virtual uint32_t onGetHeight() const = 0;

        virtual bool needGlyphPath(const Paint& paint);

        virtual void onUpdateViewport(uint32_t width, uint32_t height) = 0;

        inline bool isDrawDebugLine() const
        {
            return draw_debug_line_;
        }

...
#include "gl/skity/render/sw/sw_subpixel.hpp"
...
namespace skity {
    ...
    {
    public:
        SWSpanBrush(const std::vector<Span>& spans, Bitmap* bitmap)
            : p_spans_(spans.data())
    ...
    protected:
        ...
        const Span* GetSpans() const
        {
            return p_spans_;
        }

        size_t GetSpanSize() const
        {
            return spans_size_;
        }
    ...
    {
    public:
        SolidColorBrush(const std::vector<Span>& spans, Bitmap* bitmap, Color4f color)
            : SWSpanBrush(spans, bitmap)

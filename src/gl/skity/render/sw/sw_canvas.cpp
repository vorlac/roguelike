...
#include "gl/skity/render/sw/sw_canvas.hpp"
#include "gl/skity/render/sw/sw_raster.hpp"
...
namespace skity {
    ...
    SWCanvas::SWCanvas(Bitmap* bitmap)
        : Canvas()
        , bitmap_(bitmap)
    {
    }

    void SWCanvas::onDrawLine(float x0, float y0, float x1, float y1, const Paint& paint)
    {
    }
    ...
    void SWCanvas::onDrawBlob(const TextBlob* blob, float x, float y, const Paint& paint)
    {
    }
    ...
    uint32_t SWCanvas::onGetWidth() const
    {
        return bitmap_->width();
    }

    uint32_t SWCanvas::onGetHeight() const
    {
    ...
    std::unique_ptr<SWSpanBrush> SWCanvas::GenerateBrush(const std::vector<Span>& spans,
                                                         const skity::Paint& paint, bool stroke)
    {

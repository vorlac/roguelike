...
namespace skity {
    ...
    void SWRaster::RastePath(const Path& path)
    {
    ...
    void SWRaster::DoRasteration()
    {
        ...
        for (int32_t current_y = min_ey_; current_y <= max_ey_; current_y++)
        {
            ...
            const auto& list = cells_y_[current_y];
    ...
    void SWRaster::RenderLine(int32_t to_x, int32_t to_y)
    {
        ...
        {  // any other line
            ...
            do
            {
                if (prod <= 0 && prod - dx * SW_ONE_PIXEL > 0)
                {  // left
                    fx2 = 0;
                    fy2 = -1 * (0 * dx - prod) / dx;
                    ...
                    ex1--;
                         ...
                         prod - dx * SW_ONE_PIXEL + dy * SW_ONE_PIXEL > 0)
                {  // up
    ...
    void SWRaster::Sweepline(int32_t x, int32_t y, int32_t area, int32_t count)
    {
        ...
        if (coverage)
        {
            if (!this->spans_.empty())
            {
                ...
                if (p_span->y == y && p_span->x + p_span->len == x && p_span->cover == coverage)
                {

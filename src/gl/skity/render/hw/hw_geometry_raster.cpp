...
namespace skity {
    ...
    HWGeometryRaster::HWGeometryRaster(HWMesh* mesh, const Paint& paint, bool use_gs)
        : mesh_(mesh)
        , paint_(paint)
        , use_gs_(use_gs)
    {
    ...
    void HWGeometryRaster::FillTextRect(const glm::vec4& bounds, const glm::vec2& uv_lt,
                                        const glm::vec2& uv_rb)
    {
        ...
        auto a = AppendVertex(left_top.x, left_top.y, HW_VERTEX_TYPE_TEXT, uv_lt.x, uv_lt.y);
        auto b = AppendVertex(left_bottom.x, left_bottom.y, HW_VERTEX_TYPE_TEXT, uv_lb.x, uv_lb.y);
        auto c = AppendVertex(top_right.x, top_right.y, HW_VERTEX_TYPE_TEXT, uv_tr.x, uv_tr.y);
        auto d = AppendVertex(right_bottom.x, right_bottom.y, HW_VERTEX_TYPE_TEXT, uv_rb.x, uv_rb.y);
    ...
    void HWGeometryRaster::HandleLineCap(const glm::vec2& center, const glm::vec2& p0,
                                         const glm::vec2& p1, const glm::vec2& out_dir,
                                         float stroke_radius)
    {
    ...
    std::array<glm::vec2, 4> HWGeometryRaster::ExpandLine(const glm::vec2& p0, const glm::vec2& p1,
                                                          float stroke_radius)
    {
    ...
    uint32_t HWGeometryRaster::AppendLineVertex(const glm::vec2& p)
    {
        ExpandBounds(p);
        return mesh_->AppendVertex(p.x, p.y, HW_VERTEX_TYPE_LINE_NORMAL);
    }

    uint32_t HWGeometryRaster::AppendCircleVertex(const glm::vec2& p, const glm::vec2& center)
    {
        ExpandBounds(p);
        return mesh_->AppendVertex(p.x, p.y, HW_VERTEX_TYPE_CIRCLE, center.x, center.y);
    }

    uint32_t HWGeometryRaster::AppendVertex(float x, float y, float mix, float u, float v)
    {
        ExpandBounds({ x, y });
        return mesh_->AppendVertex(x, y, mix, u, v);
    }

    void HWGeometryRaster::AppendRect(uint32_t a, uint32_t b, uint32_t c, uint32_t d)
    {
        /**
         *   a --------- c
         *   |           |
    ...
    void HWGeometryRaster::ExpandBounds(const glm::vec2& p)
    {
    ...
    void HWGeometryRaster::FillRect(const Rect& rect)
    {
    ...
    void HWGeometryRaster::StrokeRect(const Rect& rect)
    {
        float stroke_width = StrokeWidth();
    ...
    void HWGeometryRaster::HandleGSRoundCap(const glm::vec2& center, const glm::vec2& p0,
                                            const glm::vec2& p1, const glm::vec2& out_dir,
                                            float stroke_radius)
    {
        glm::vec2 a = p0 + out_dir * stroke_radius;
    ...
    void HWGeometryRaster::GenerateCircleMesh(const Vec2& center, const Vec2& p1, const Vec2& p2)
    {

...
#include "gl/skity/render/hw/hw_canvas.hpp"
#include "gl/skity/render/hw/hw_mesh.hpp"
#include "gl/skity/render/hw/hw_path_raster.hpp"
...
#ifdef SKITY_VULKAN
  #include "gl/skity/render/hw/vk/vk_canvas.hpp"
#endif

namespace skity {

    std::unique_ptr<Canvas> Canvas::MakeHardwareAccelationCanvas(uint32_t width, uint32_t height,
                                                                 float density, GPUContext* ctx)
    {
        glm::mat4 mvp;
        if (ctx->type == GPUBackendType::kOpenGL || ctx->type == GPUBackendType::kWebGL2)
    ...
    HWCanvas::~HWCanvas()
    {
        for (const auto& it : image_texture_store_)
            it.second->Destroy();

        for (const auto& it : font_texture_store_)
            it.second->Destroy();
    ...
    uint32_t HWCanvas::onGetWidth() const
    {
        return width_;
    }

    uint32_t HWCanvas::onGetHeight() const
    {
        return height_;
    }
    ...
    std::unique_ptr<HWDraw> HWCanvas::GenerateColorOp(const Paint& paint, bool stroke,
                                                      const Rect& bounds)
    {
        ...
        if (shader)
        {
            ...
            if (gradient_type != Shader::kNone)
            {
                ...
                {
                    draw->SetGradientBounds(gradient_info.point[0],
            ...
            else if (pixmap)
            {
                ...
                float width = std::min<float>(bounds.width(), static_cast<float>(pixmap->Width()));
    ...
    void HWCanvas::onClipPath(const Path& path, ClipOp op)
    {
        ...
        {
            // convex polygon save
            state_.SaveClipPath({ raster.ColorStart(), raster.ColorCount() },
        ...
        {
            draw->SetStencilRange({ raster.StencilFrontStart(), raster.StencilFrontCount() },
                                  { raster.StencilBackStart(), raster.StencilBackCount() });
        }
    ...
    void HWCanvas::onDrawLine(float x0, float y0, float x1, float y1, const Paint& paint)
    {
    ...
    void HWCanvas::onDrawRect(const Rect& rect, const Paint& paint)
    {
        if (rect.isEmpty())
            return;
    ...
    void HWCanvas::onDrawBlob(const TextBlob* blob, float x, float y, const Paint& paint)
    {
        bool need_fill = paint.getStyle() != Paint::kStroke_Style;
        bool need_stroke = paint.getStyle() != Paint::kFill_Style;
        ...
        if (need_fill)
        {
            ...
            float offset_x = 0.f;
            for (const auto& run : blob->getTextRun())
                if (need_path_fill)
    ...
    void HWCanvas::onTranslate(float dx, float dy)
    {
        state_.Translate(dx, dy);
    }

    void HWCanvas::onScale(float sx, float sy)
    {
        state_.Scale(sx, sy);
    }

    void HWCanvas::onRotate(float degree)
    {
        state_.Rotate(degree);
    }
    ...
    void HWCanvas::onConcat(const Matrix& matrix)
    {
        state_.Concat(matrix);
    }
    ...
    HWTexture* HWCanvas::QueryTexture(Pixmap* pixmap)
    {
        ...
        texture->UploadData(0, 0, pixmap->Width(), pixmap->Height(), (void*)pixmap->Addr());
    ...
    HWRenderTarget* HWCanvas::QueryRenderTarget(const Rect& bounds)
    {
        ...
        return render_target_cache_.StoreCache(GenerateBackendRenderTarget(width, height));
    }

    float HWCanvas::FillTextRun(float x, float y, const TextRun& run, const Paint& paint)
    {
        ...
        for (const auto& info : run.getGlyphInfo())
        {
            auto region = font_texture->GetGlyphRegion(info.id, info.font_size * density_);
        ...
        auto draw = GenerateColorOp(paint, false, skity::Rect::MakeXYWH(x, y, offset_x, max_height));
    ...
    float HWCanvas::FillTextRunWithPath(float x, float y, const TextRun& run, const Paint& paint)
    {
        auto typeface = run.lockTypeface();
        ...
        for (const auto& info : run.getGlyphInfo())
        {
            ...
            glm::mat4 transform = glm::translate(glm::identity<glm::mat4>(), { offset_x, y, 0.f });
    ...
    float HWCanvas::StrokeTextRun(float x, float y, const TextRun& run, const Paint& paint)
    {
        auto typeface = run.lockTypeface();
        ...
        for (const auto& info : run.getGlyphInfo())
        {
            ...
            auto draw = GenerateColorOp(paint, true,
                                        Rect::MakeXYWH(offset_x, y, info.advance_x, max_height));
    ...
    void HWCanvas::ForwardFillClipMask()
    {
        state_.ForEachClipStackValue(
            [this](const HWCanvasState::ClipStackValue& clip_value, size_t i) {
                bool has_clip = i != 0;
                auto draw = std::make_unique<HWDraw>(GetPipeline(), has_clip, true);
    ...
    void HWCanvas::EnqueueDrawOp(std::unique_ptr<HWDraw> draw, const Rect& bounds,
                                 const std::shared_ptr<MaskFilter>& mask_filter)
    {
        if (mask_filter)
        {
            ...
            auto op = std::make_unique<PostProcessDraw>(fbo, std::move(draw), filter_bounds,
                                                        GetPipeline(), state_.HasClip());
    ...
    void HWCanvas::HandleMaskFilter(DrawList draw_list, const Rect& bounds,
                                    const std::shared_ptr<MaskFilter>& mask_filter)
    {
        if (mask_filter)
        {
            ...
            auto op = std::make_unique<PostProcessDraw>(fbo, std::move(draw_list), filter_bounds,
                                                        GetPipeline(), state_.HasClip());

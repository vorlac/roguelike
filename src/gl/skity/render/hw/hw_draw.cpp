...
namespace skity {
    ...
    void HWDraw::SetPipelineColorMode(uint32_t mode)
    {
        pipeline_mode_ = mode;
    }

    void HWDraw::SetStencilRange(const HWDrawRange& front_range, const HWDrawRange& back_range)
    {
        stencil_front_range_ = front_range;
        stencil_back_range_ = back_range;
    }

    void HWDraw::SetColorRange(const HWDrawRange& color_range)
    {
    ...
    {
        stroke_width_.Set(width);
    }

    void HWDraw::SetUniformColor(const glm::vec4& color)
    {
        uniform_color_.Set(color);
    }

    void HWDraw::SetTransformMatrix(const glm::mat4& matrix)
    {
        transform_matrix_.Set(matrix);
    }

    void HWDraw::SetGradientBounds(const glm::vec2& p0, const glm::vec2& p1)
    {
        gradient_bounds_.Set(glm::vec4{ p0, p1 });
    }

    void HWDraw::SetGradientColors(const std::vector<glm::vec4>& colors)
    {
        gradient_colors_ = colors;
    }

    void HWDraw::SetGradientPositions(const std::vector<float>& pos)
    {
        gradient_stops_ = pos;
    }

    void HWDraw::SetClearStencilClip(bool clear)
    {
        clear_stencil_clip_ = clear;
    }

    void HWDraw::SetTexture(HWTexture* texture)
    {
        texture_ = texture;
    }
    ...
    void HWDraw::DoStencilIfNeed()
    {
        ...
        if (stencil_front_range_.count > 0)
        {
            ...
            renderer_->DrawIndex(stencil_front_range_.start, stencil_front_range_.count);
        }
    ...
    void HWDraw::DoColorFill()
    {
        ...
        else if (pipeline_mode_ == kLinearGradient || pipeline_mode_ == kRadialGradient)
        {
            renderer_->SetPipelineColorMode((HWPipelineColorMode)pipeline_mode_);
            renderer_->SetGradientCountInfo(gradient_colors_.size(), gradient_stops_.size());
        ...
        else if (pipeline_mode_ >= kImageTexture || pipeline_mode_ <= kInnerBlurMix)
        {
            renderer_->SetPipelineColorMode((HWPipelineColorMode)pipeline_mode_);
    ...
    {
        bool has_stencil_discard = stencil_back_range_.count != 0 ||
                                   stencil_front_range_.count != 0;
    ...
    PostProcessDraw::PostProcessDraw(
        HWRenderTarget* render_target, std::vector<std::unique_ptr<HWDraw>> draw_list,
        const Rect& bounds, HWRenderer* pipeline, bool has_clip, bool clip_stencil)
    ...
    PostProcessDraw::PostProcessDraw(HWRenderTarget* render_target, std::unique_ptr<HWDraw> op,
                                     const Rect& bounds, HWRenderer* pipeline, bool has_clip,
                                     bool clip_stencil)
        : HWDraw(pipeline, has_clip, clip_stencil)
        , render_target_(render_target)
        , bounds_(bounds)
    ...
    void PostProcessDraw::DrawToRenderTarget()
    {
        ...
        auto mvp = glm::ortho(bounds_.left(), bounds_.right(), bounds_.bottom(), bounds_.top());
    ...
    {
        const auto& matrix = TransformMatrix();

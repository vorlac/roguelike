...
namespace skity {
    ...
    void GLTexture::Resize(uint32_t width, uint32_t height)
    {
        ...
        {
            GL_CALL(TexImage2D, GL_TEXTURE_2D, 0, GetInternalFormat(), width, height, 0, format_,
                    GetInternalType(), nullptr);
        }
    }
                               ...
                               uint32_t height, void* data)
    {
        ...
        GL_CALL(TexSubImage2D, GL_TEXTURE_2D, 0, offset_x, offset_y, width, height, format_,
                GetInternalType(), data);
    }

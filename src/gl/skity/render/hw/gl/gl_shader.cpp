...
namespace skity {
    ...
    GLuint create_shader(const char* source, GLenum type, bool use_gs = false)
    {
        ...
        std::array<const char*, 2> shaders{ "#version 330 core \n", source };
    ...
    GLuint create_shader_program(const char* vs_code, const char* fs_code, const char* gs_code)
    {
        GLuint program = GL_CALL(CreateProgram);
        GLint success;
    ...
    void GLShader::SetUniform(int32_t location, const glm::vec4& value)
    {
    ...
    void GLShader::SetUniform(int32_t location, const glm::vec3& value)
    {
        GL_CALL(Uniform3f, location, value[0], value[1], value[2]);
    }

    void GLShader::SetUniform(int32_t location, const glm::vec2& value)
    {
        GL_CALL(Uniform2f, location, value[0], value[1]);
    }

    void GLShader::SetUniform(int32_t location, const glm::mat4& value)
    {
    ...
    std::unique_ptr<GLPipelineShader> GLShader::CreatePipelineShader()
    {
        ...
        std::string fs_shader{ (const char*)hw_gl_fragment_glsl, hw_gl_fragment_glsl_size };
    ...
    std::unique_ptr<GLPipelineShader> GLShader::CreateGSPipelineShader()
    {
        ...
        std::string vs_shader{ (const char*)hw_gl_gs_vertex_glsl, hw_gl_gs_vertex_glsl_size };
        std::string fs_shader{ (const char*)hw_gl_gs_fragment_glsl, hw_gl_gs_fragment_glsl_size };
        std::string gs_shader{ (const char*)hw_gl_gs_geometry_glsl, hw_gl_gs_geometry_glsl_size };
    ...
    void GLPipelineShader::SetGradientCountInfo(int32_t color_count, int32_t pos_count)
    {
    ...
    {
        SetUniform(gradient_colors_location_, (glm::vec4*)colors.data(), (int32_t)colors.size());
    }

...
namespace skity {
    ...
    void GLRenderTarget::BlitColorTexture()
    {
        ...
        GL_CALL(BindFramebuffer, GL_FRAMEBUFFER, fbo_);
        GL_CALL(FramebufferTexture2D, GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                color_texture_.GetTextureID(), 0);
    ...
    {
        GL_CALL(FramebufferTexture2D, GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                horizontal_texture_.GetTextureID(), 0);
    ...
    {
        GL_CALL(FramebufferTexture2D, GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                vertical_texture_.GetTextureID(), 0);
    ...
    void GLRenderTarget::InitTextures()
    {
        ...
        horizontal_texture_.Init(HWTexture::Type::kColorTexture, HWTexture::Format::kRGBA);
        horizontal_texture_.Bind();
        horizontal_texture_.Resize(Width(), Height());

        // init vertical texture
        vertical_texture_.Init(HWTexture::Type::kColorTexture, HWTexture::Format::kRGBA);
        ...
        stencil_texture_.Init(HWTexture::Type::kStencilTexture, HWTexture::Format::kS);
    ...
    {
        const static GLenum color_buffer = GL_COLOR_ATTACHMENT0;
        const static GLenum stencil_buffer = GL_DEPTH_STENCIL_ATTACHMENT;

        const static float transparent[] = { 0.f, 0.f, 0.f, 0.f };

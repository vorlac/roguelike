...
#include "gl/skity/render/hw/gl/gl_renderer.hpp"
...
namespace skity {
    ...
    void GLRenderer::SetViewProjectionMatrix(const glm::mat4& mvp)
    {
        HWRenderer::SetViewProjectionMatrix(mvp);
        shader_->SetMVP(mvp);
    }

    void GLRenderer::SetModelMatrix(const glm::mat4& matrix)
    {
    ...
    void GLRenderer::SetStrokeWidth(float width)
    {
    ...
    void GLRenderer::UploadIndexBuffer(void* data, size_t data_size)
    {
        ...
        if (data_size > buffer_sizes_[1])
        {
            ...
            GL_CALL(BufferData, GL_ELEMENT_ARRAY_BUFFER, data_size, nullptr, GL_STREAM_DRAW);
        }
    ...
    void GLRenderer::SetGlobalAlpha(float alpha)
    {
        shader_->SetGlobalAlpha(alpha);
    }

    void GLRenderer::EnableStencilTest()
    {
    ...
    void GLRenderer::DisableColorOutput()
    {
        GL_CALL(ColorMask, 0, 0, 0, 0);
    }

    void GLRenderer::EnableColorOutput()
    {
    ...
    void GLRenderer::UpdateStencilFunc(HWStencilFunc func, uint32_t value, uint32_t compare_mask)
    {
    ...
    {
        GL_CALL(DrawElements, GL_TRIANGLES, count, GL_UNSIGNED_INT, (void*)(start * sizeof(GLuint)));
    }
    ...
    void GLRenderer::InitBufferObject()
    {
        ...
        GL_CALL(VertexAttribPointer, 0, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);

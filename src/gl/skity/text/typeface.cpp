...
namespace skity {
    ...
    private:
        GlyphInfo ScaleInfo(const GlyphInfo& base_info, float target_font_size, bool load_path)
        {
    ...
    std::shared_ptr<Typeface> Typeface::MakeFromData(const std::shared_ptr<Data>& data)
    {
#ifdef ENABLE_TEXT_RENDER
        std::unique_ptr<Typeface::Impl> impl{ new Typeface::Impl };
        impl->SetFTLibrary(GlobalFTLibrary());
    ...
    void Typeface::getGlyphInfo(const std::vector<GlyphID>& glyph_id, float font_size,
                                std::vector<GlyphInfo>& info, bool load_path)
    {
        for (auto id : glyph_id)
            info.emplace_back(getGlyphInfo(id, font_size, load_path));
    }

    GlyphInfo Typeface::getGlyphInfo(GlyphID glyph_id, float font_size, bool load_path)
    {
#ifdef ENABLE_TEXT_RENDER
    ...
    GlyphBitmapInfo Typeface::getGlyphBitmapInfo(GlyphID glyph_id, float font_size)
    {
#ifdef ENABLE_TEXT_RENDER

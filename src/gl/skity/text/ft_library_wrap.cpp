...
#include "gl/skity/io/data.hpp"
#include "gl/skity/logging.hpp"
#include "gl/skity/text/ft_library_wrap.hpp"
...
namespace skity {
    ...
    static int HandleConicTo(const FT_Vector* control, const FT_Vector* to, void* user)
    {
        auto* info = static_cast<FTOutlineExtractInfo*>(user);
        info->path->quadTo(FTUnitToPixel(control->x, info), FTUnitToPixel(control->y, info),
    ...
    {
        auto* info = static_cast<FTOutlineExtractInfo*>(user);
        info->path->cubicTo(FTUnitToPixel(control1->x, info), FTUnitToPixel(control1->y, info),
    ...
    {
        FT_Face ft_face;
        FT_Error error = FT_New_Memory_Face(ft_library_, (const FT_Byte*)data->RawData(),
                                            data->Size(), 0, &ft_face);
                                                   ...
                                                   float canvasWidth, float canvasHeight)
    {
        ...
        std::wstring_convert<std::codecvt<char32_t, char8_t, char32_t>, char8_t> utf32_conv;
        ...
        for (char32_t c : utf32)
        {
            ...
            FT_Error error = FT_Load_Glyph(ft_face_, index, FT_LOAD_NO_SCALE | FT_LOAD_NO_BITMAP);

            if (error)
            {
                // TODO: why can't it print this?
                // LOG_ERROR("failed to load Glyph at index {:x}", c, index);
                continue;
            }
    ...
    FTGlyphInfo FTTypeFace::LoadGlyph(GlyphID glyph_id, float font_size, bool load_path)
    {
        FTGlyphInfo info;
        ...
        FilpOutline();
        info.advance_x = ft_face_->glyph->advance.x * current_font_size_ / ft_face_->units_per_EM;
        info.advance_y = ft_face_->glyph->advance.y * current_font_size_ / ft_face_->units_per_EM;
        info.width = ft_face_->glyph->metrics.width * current_font_size_ / ft_face_->units_per_EM;
        info.height = ft_face_->glyph->metrics.height * current_font_size_ / ft_face_->units_per_EM;
    ...
    Path FTTypeFace::ExtractOutLine()
    {
        ...
        FT_Error error = FT_Outline_Decompose(&ft_face_->glyph->outline, &callback, &outline_info);
    ...
    FTGlyphBitmapInfo FTTypeFace::LoadGlyphBitmap(GlyphID glyph_id, float font_size)
    {
        if (!FloatNearlyZero(current_font_size_ - font_size))
        {
            current_font_size_ = font_size;

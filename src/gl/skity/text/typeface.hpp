...
namespace skity {
    ...
    public:
         ...
         * @return
         */
        static std::shared_ptr<Typeface> MakeFromData(const std::shared_ptr<Data>& data);

        void getGlyphInfo(const std::vector<GlyphID>& glyph_id, float font_size,
                          std::vector<GlyphInfo>& info, bool load_path = false);

        GlyphInfo getGlyphInfo(GlyphID glyph_id, float font_size, bool load_path = false);

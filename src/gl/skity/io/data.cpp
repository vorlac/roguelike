...
namespace skity {
    ...
    std::shared_ptr<Data> Data::PrivateNewWithCopy(const void* srcOrNull, size_t length)
    {
        if (0 == length)
    ...
    std::shared_ptr<Data> Data::MakeFromFileName(const char path[])
    {
        ...
        std::vector<uint8_t> raw_file_data((std::istreambuf_iterator<char>(in_stream)),
                                           std::istreambuf_iterator<char>());

        return MakeWithCopy(raw_file_data.data(), raw_file_data.size());
    }

    std::shared_ptr<Data> Data::MakeWithProc(const void* ptr, size_t length, ReleaseProc proc,
                                             void* ctx)
    {
    ...
    {
        return std::shared_ptr<Data>(new Data(data, length, skity_free_releaseproc, nullptr));
    }

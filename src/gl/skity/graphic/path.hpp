...
namespace skity {
    ...
    public:
        ...
        public:
             ...
             */
            Iter(const Path& path, bool forceClose);

            ~Iter() = default;

            void setPath(const Path& path, bool forceClose);
        ...
        public:
            ...
            explicit RawIter(const Path& path)
                : RawIter()
            {
                setPath(path);
            }
        ...
        inline size_t countPoints() const
        {
            return points_.size();
        }

        inline size_t countVerbs() const
        {
            return verbs_.size();
        }
        ...
        Path& quadTo(const Point& p1, const Point& p2)
        {
        ...
        Path& conicTo(const Point& p1, const Point& p2, float weight)
        {
        ...
        Path& cubicTo(const Point& p1, const Point& p2, const Point& p3)
        {
         ...
         */
        Path& arcTo(const Rect& oval, float startAngle, float sweepAngle, bool forceMove);
         ...
         */
        Path& arcTo(float rx, float ry, float xAxisRotate, ArcSize largeArc, Direction sweep,
                    float x, float y);
        Path& close();
        Path& reset();
         ...
         */
        Path& addCircle(float x, float y, float radius, Direction dir = Direction::kCW);
         ...
         */
        Path& addRect(const Rect& rect, Direction dir, uint32_t start);
        ...
        Path& addRoundRect(const Rect& rect, float rx, float ry, Direction dir = Direction::kCW);

        Path& addRoundRect(const Rect& rect, const float radii[], Direction dir = Direction::kCW);

        Path& addRRect(const RRect& rrect, Direction dir = Direction::kCW);

        Path& addRRect(const RRect& rrect, Direction dir, uint32_t start);
        /**
         ...
         * @return
         */
        Path& addPath(const Path& src, float dx, float dy, AddMode mode = AddMode::kAppend);
        Path& addPath(const Path& src, AddMode mode = AddMode::kAppend);
        Path& addPath(const Path& src, const Matrix& matrix, AddMode mode = AddMode::kAppend);
        /**
        ...
        void setLastPt(const Point& p)
        {
            this->setLastPt(p.x, p.y);
        }

        inline Direction getFirstDirection()
        {
            return first_direction_;
        }

        inline void setFirstDirection(Direction dir)
        {
            this->first_direction_ = dir;
        }
        ...
        const Verb* verbsBegin() const
        {
            return verbs_.data();
        }

        const Verb* verbsEnd() const
        {
            return verbs_.data() + countVerbs();
        }
        ...
        const float* conicWeights() const
        {
            return conic_weights_.data();
        }
    ...
    private:
        ...
        const inline Point& atPoint(int32_t index) const
        {
        ...
        static bool ComputePtBounds(Rect* bounds, const Path& ref);

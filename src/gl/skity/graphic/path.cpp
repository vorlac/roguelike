...
namespace skity {
    ...
    static bool arc_is_long_point(const Rect& oval, float start_angle, float sweep_angle, Point* pt)
    {
    ...
    static std::tuple<Vec2, Vec2, RotationDirection> angles_to_unit_vectors(float startAngle,
                                                                            float sweepAngle)
    {
        float startRad = glm::radians(startAngle), stopRad = glm::radians(startAngle + sweepAngle);
     ...
     */
    static int build_arc_conics(const Rect& oval, const Vec2& start, const Vec2& stop,
                                RotationDirection dir, Conic conics[Conic::kMaxConicsForArc],
                                Point* singlePt)
    {
        Matrix matrix = glm::translate(glm::mat4(1.f), Vec3{ oval.centerX(), oval.centerY(), 0.f }) *
                        glm::scale(glm::mat4(1.f),
                                   Vec3{ oval.width() * 0.5f, oval.height() * 0.5f, 1.f });

        int count = Conic::BuildUnitArc(Vec4{ start, 0, 1 }, Vec4{ stop, 0, 1 }, dir, &matrix,
                                        conics);
        if (0 == count)
    ...
    {
    public:
        Path_RectPointIterator(const Rect& rect, Path::Direction dir, size_t startIndex)
    ...
    {
    public:
        Path_OvalPointIterator(const Rect& oval, Path::Direction dir, size_t startIndex)
            : Path_PointIterator<4>(dir, startIndex)
        {
    ...
    {
    public:
        Path_RRectPointIterator(const RRect& rrect, Path::Direction dir, uint32_t start_index)
    ...
    public:
        explicit AutoDisableDirectionCheck(Path* p)
            : path{ p }
            , saved{ p->getFirstDirection() }
        {
        }

        ~AutoDisableDirectionCheck()
        {
            path->setFirstDirection(saved);
        }
    ...
    {
    public:
        AutoPathBoundsUpdate(Path* p, const Rect& r)
            : path{ p }
            , rect{ r }
        {
            rect.sort();
    ...
    private:
        ...
        static bool is_degenerate(const Path& path)
        {
    ...
    Path::Iter::Iter(const Path& path, bool forceClose)
        : Iter()
    {
        this->setPath(path, forceClose);
    }

    void Path::Iter::setPath(const Path& path, bool forceClose)
    {
    ...
    float Path::Iter::conicWeight() const
    {
    ...
    Path::Verb Path::Iter::autoClose(Point* pts)
    {
        ...
        {
            if (FloatIsNan(last_pt_.x) || FloatIsNan(last_pt_.y) || FloatIsNan(move_to_.x) ||
                FloatIsNan(move_to_.y))
            {
                return Verb::kClose;
            }
    ...
    bool Path::Iter::isCloseLine() const
    {
        return close_line_;
    }
    ...
    float Path::RawIter::conicWeight() const
    {
        return *conic_weights_;
    }
    ...
    Path& Path::arcTo(const Rect& oval, float startAngle, float sweepAngle, bool forceMove)
    {
    ...
    Path& Path::arcTo(float rx, float ry, float xAxisRotate, ArcSize largeArc, Direction sweep,
                      float x, float y)
    {
        ...
        Matrix point_transform = glm::rotate(glm::identity<Matrix>(), -xAxisRotate,
                                             glm::vec3(0, 0, 1.f));
        ...
        point_transform = glm::scale(glm::identity<Matrix>(), glm::vec3(1.f / rx, 1.f / ry, 1.f));

        point_transform = point_transform * glm::rotate(glm::identity<Matrix>(), -xAxisRotate,
        ...
        int segments = std::ceil(std::abs(theta_arc / (2.f * glm::pi<float>() / 3.f)));
        ...
        bool expect_integers = FloatNearlyZero(glm::pi<float>() / 2.f - std::abs(theta_width)) &&
                               float_is_integer(rx) && float_is_integer(ry) &&
                               float_is_integer(x) && float_is_integer(y);
    ...
    Path& Path::addRect(const Rect& rect, Direction dir, uint32_t start)
    {
    ...
    Path& Path::close()
    {
        ...
        last_move_to_index_ ^= ~last_move_to_index_ >> (8 * sizeof(last_move_to_index_) - 1);

        return *this;
    }

    Path& Path::addRoundRect(const Rect& rect, float rx, float ry, Direction dir)
    {
        if (rx < 0 || ry < 0)
            return *this;
    ...
    Path& Path::addRRect(const RRect& rrect, Direction dir)
    {
        // legacy start indices: 6 (CW) and 7(CCW)
        return this->addRRect(rrect, dir, dir == Direction::kCW ? 6 : 7);
    }

    Path& Path::addRRect(const RRect& rrect, Direction dir, uint32_t start_index)
    {
        ...
        else
        {
            ...
            int verbs = starts_with_conic ? 9    // moveTo + 4x conicTo + 3x lineTo + close
            ...
            uint32_t rect_start_index = start_index / 2 + (dir == Direction::kCW ? 0 : 1);
    ...
    {
        if (radius > 0)
            addOval(Rect::MakeLTRB(x - radius, y - radius, x + radius, y + radius), dir);
    ...
    Path& Path::addOval(const Rect& oval, Direction dir, uint32_t start)
    {
        ...
        Path_RectPointIterator rect_iter{ oval, dir, start + (dir == Direction::kCW ? 0 : 1) };
    ...
    Path& Path::reversePathTo(const Path& src)
    {
        ...
        const float* conic_weights = src.conic_weights_.data() + src.conic_weights_.size();
    ...
    bool Path::operator==(const Path& other)
    {
        return (this == std::addressof(other)) ||
               (last_move_to_index_ == other.last_move_to_index_ &&
                ...
                points_.data() == other.points_.data() && verbs_.data() == other.verbs_.data() &&
    ...
    void Path::injectMoveToIfNeed()
    {
        if (last_move_to_index_ < 0)
        {
            ...
            {
                const Point& pt = atPoint(~last_move_to_index_);
                x = pt.x;
                y = pt.y;
            }
            moveTo(x, y);
    ...
    {
        for (auto it : verbs_)
            if (it == Verb::kLine || it == Verb::kQuad || it == Verb::kConic || it == Verb::kCubic)
                return false;
        return true;
    }
    ...
    static void append_params(std::ostream& os, const std::string& label, const Point pts[],
                              int count, float conicWeight = -12345)
    {
        os << label << "(";
        for (int i = 0; i < count; i++)
        {

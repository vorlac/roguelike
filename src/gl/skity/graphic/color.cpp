...
namespace skity {
    ...
    Color ColorMakeFromHSLA(float h, float s, float l, uint8_t a)
    {
        ...
        auto r = static_cast<uint8_t>(glm::clamp(hsla_hue(h + 1.f / 3.f, m1, m2), 0.f, 1.f) * 255.f);
        auto g = static_cast<uint8_t>(glm::clamp(hsla_hue(h, m1, m2), 0.f, 1.f) * 255.f);
        auto b = static_cast<uint8_t>(glm::clamp(hsla_hue(h - 1.f / 3.f, m1, m2), 0.f, 1.f) * 255.f);

...
namespace skity {
    ...
    public:
        struct Iterate
        {
            Iterate(const Path& path)
                : Iterate(path.verbsBegin(),
                          (!path.isFinite()) ? path.verbsBegin() : path.verbsEnd(), path.points(),
                          path.conicWeights())
            {
            }

            Iterate(const Path::Verb* verbs_begin, const Path::Verb* verbs_end, const Point* points,
                    const float* weights)

...
namespace skity {
    ...
    public:
         ...
         */
        bool filterPath(Path* dst, const Path& src, bool stroke, const Paint& paint) const;
         ...
         */
        static std::shared_ptr<PathEffect> MakeDiscretePathEffect(float seg_length, float dev,
                                                                  uint32_t seed_assist = 0);

        static std::shared_ptr<PathEffect> MakeDashPathEffect(const float intervals[], int count,
                                                              float phase);

    protected:
        PathEffect() = default;

        virtual bool onFilterPath(Path*, const Path&, bool, const Paint&) const = 0;

...
namespace skity {
    ...
    static void Perterb(Point* p, const Vector& tangent, float scale)
    {
    ...
    public:
        ...
        */
        float nextSScalar1()
        {
            return FixedToFloat(this->nextSFixed1());
        }
    ...
    DiscretePathEffect::DiscretePathEffect(float seg_length, float deviation, uint32_t seed_assist)
        : PathEffect()
    ...
    bool DiscretePathEffect::onFilterPath(Path* dst, const Path& src, bool stroke,
                                          const Paint&) const
    {
        bool do_fill = !stroke;
        ...
        uint32_t seed = seed_assist_ ^ static_cast<int32_t>(std::round(meas.getLength()));

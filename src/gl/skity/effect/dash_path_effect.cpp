...
#include "gl/skity/graphic/path_measure.hpp"
...
#define kMaxDashCount 1000000
    ...
    static float find_first_interval(const float intervals[], float phase, int32_t* index,
                                     int32_t count)
    {
    ...
    public:
        ...
        bool Init(const Path& src, Path* dst, const Paint& paint, int32_t interval_count,
    ...
    DashPathEffect::DashPathEffect(const float* intervals, int32_t count, float phase)
        : PathEffect()
        , phase_(0)
                                      ...
                                      const Paint& paint) const
    {
        ...
        do
        {
            ...
            // initial segment
            if (meas.isClosed() && is_even(initial_dash_index_) && initial_dash_length_ >= 0)
            {
    ...
    void DashPathEffect::CalcDashParameters(float phase)
    {
        ...
        initial_dash_length_ = find_first_interval(intervals_.get(), phase, &initial_dash_index_,
                                                   count_);

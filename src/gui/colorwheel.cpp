...
namespace rl::gui {
    ...
    {
        float x = p.x() - m_pos.x(), y = p.y() - m_pos.y(), w = m_size.x(), h = m_size.y();
        ...
        float a = -m_hue * 2 * NVG_PI, sin_a = std::sin(a), cos_a = std::cos(a);
        Vector2f xy(cos_a * x - sin_a * y, sin_a * x + cos_a * y);

        float r = r0 - 6;
        float l0 = (r - xy.x() + std::sqrt(3) * xy.y()) / (3 * r);
        float l1 = (r - xy.x() - std::sqrt(3) * xy.y()) / (3 * r);
        float l2 = 1 - l0 - l1;
        bool triangle_test = l0 >= 0 && l0 <= 1.f && l1 >= 0.f && l1 <= 1.f && l2 >= 0.f &&
                             l2 <= 1.f;

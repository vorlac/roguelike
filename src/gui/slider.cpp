...
namespace rl::gui {
    ...
    void Slider::draw(NVGcontext* ctx)
    {
        ...
        NVGpaint bg = nvgBoxGradient(ctx, start_x, center.y() - 3 + 1, width_x, 6, 3, 3,
                                     Color(0, m_enabled ? 32 : 10),
                                     Color(0, m_enabled ? 128 : 210));
        ...
        if (m_highlighted_range.second != m_highlighted_range.first)
        {
            nvgBeginPath(ctx);
            nvgRoundedRect(
                ctx, start_x + m_highlighted_range.first * m_size.x(), center.y() - kshadow + 1,
                width_x * (m_highlighted_range.second - m_highlighted_range.first), kshadow * 2, 2);
            nvgFillColor(ctx, m_highlight_color);
            nvgFill(ctx);
        }

        NVGpaint knob_shadow = nvgRadialGradient(ctx, knob_pos.x(), knob_pos.y(), kr - kshadow,
                                                 kr + kshadow, Color(0, 64),
        ...
        NVGpaint knob = nvgLinearGradient(ctx, m_pos.x(), center.y() - kr, m_pos.x(),
                                          center.y() + kr, m_theme->m_border_light,
                                          m_theme->m_border_medium);
        NVGpaint knob_reverse = nvgLinearGradient(ctx, m_pos.x(), center.y() - kr, m_pos.x(),
                                                  center.y() + kr, m_theme->m_border_medium,

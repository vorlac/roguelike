...
namespace rl::gui {
    ...
    static void gl_map_texture_format(
        Texture::PixelFormat& pixel_format, Texture::ComponentFormat& component_format,
    ...
    void Texture::init()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            switch (i == 0 ? m_min_interpolation_mode : m_mag_interpolation_mode)
            {
                ...
                default:
                    throw std::runtime_error("Texture::Texture(): invalid interpolation mode!");
            }
        }
        ...
        switch (m_wrap_mode)
        {
            ...
            case WrapMode::MirrorRepeat:
                ...
                break;
            default:
                throw std::runtime_error("Texture::Texture(): invalid wrap mode!");
        }

        GLenum pixel_format_gl, component_format_gl, internal_format_gl;

        gl_map_texture_format(m_pixel_format, m_component_format, pixel_format_gl,
                              component_format_gl, internal_format_gl);

        (void)pixel_format_gl;
        (void)component_format_gl;
        ...
        else if (m_flags & (uint8_t)TextureFlags::RenderTarget)
        {
            ...
            if (m_samples == 1)
                CHK(glRenderbufferStorage(GL_RENDERBUFFER, internal_format_gl, (GLsizei)m_size.x(),
                                          (GLsizei)m_size.y()));
            else
        ...
        if (m_texture_handle != 0)
        {
            ...
            if (m_samples == 1)
                CHK(glTexImage2D(tex_mode, 0, internal_format_gl, (GLsizei)m_size.x(),
                                 (GLsizei)m_size.y(), 0, pixel_format_gl, component_format_gl,
                                 data));
            else
    ...
    void Texture::upload_sub_region(const uint8_t* data, const Vector2i& origin,
                                    const Vector2i& size)
    {
        if (m_samples > 1 && data != nullptr)
            throw std::runtime_error(
                "Texture::upload_sub_region(): only implemented for samples=1!");
        ...
        if (m_texture_handle == 0)
            throw std::runtime_error(
                "Texture::upload_sub_region(): not implemented for render targets!");
    ...
    void Texture::download(uint8_t* data)
    {
        ...
        if (m_flags & (uint8_t)TextureFlags::RenderTarget)
        {
            ...
            uint8_t *low = (uint8_t*)data, *high = low + (m_size.y() - 1) * stride;
        ...
        GLenum& pixel_format_gl, GLenum& component_format_gl, GLenum& internal_format_gl)
    {
            ...
            case PixelFormat::R:
                ...
                switch (component_format)
                {
                    ...
                    case ComponentFormat::Int8:
                        internal_format_gl = GL_R8_SNORM;
                        break;
                    case ComponentFormat::Int16:
                        internal_format_gl = GL_R16_SNORM;
                        break;
#endif
                    default:
                        break;
...
#if defined(NANOGUI_USE_OPENGL)
                    case ComponentFormat::Int8:
                        internal_format_gl = GL_RG8_SNORM;
                        break;
                    case ComponentFormat::Int16:
                        internal_format_gl = GL_RG16_SNORM;
                        break;
#endif
                    default:
                        break;
...
#if defined(NANOGUI_USE_OPENGL)
                    case ComponentFormat::Int8:
                        internal_format_gl = GL_RGB8_SNORM;
                        break;
                    case ComponentFormat::Int16:
                        internal_format_gl = GL_RGB16_SNORM;
                        break;
#endif
                    default:
                        break;
...
#if defined(NANOGUI_USE_OPENGL)
                    case ComponentFormat::Int8:
                        internal_format_gl = GL_RGBA8_SNORM;
                        break;
                    case ComponentFormat::Int16:
                        internal_format_gl = GL_RGBA16_SNORM;
                        break;
#endif
                    default:
                        break;
        ...
        {
            case ComponentFormat::Int8:
                component_format_gl = GL_BYTE;
                break;
            case ComponentFormat::UInt8:
                component_format_gl = GL_UNSIGNED_BYTE;
                break;
            case ComponentFormat::Int16:
                component_format_gl = GL_SHORT;
                break;
            case ComponentFormat::UInt16:
                component_format_gl = GL_UNSIGNED_SHORT;
                break;
            case ComponentFormat::Float16:
                component_format_gl = GL_HALF_FLOAT;
                break;
            case ComponentFormat::Float32:
                component_format_gl = GL_FLOAT;
                break;
            default:
                break;
        }
        ...
        if (internal_format_gl == 0)
            throw std::runtime_error(
                "gl_map_texture_format(): component format unsupported "

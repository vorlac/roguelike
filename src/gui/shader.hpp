...
namespace rl::gui {
    ...
    public:
         ...
         */
        Shader(RenderPass* render_pass, const std::string& name, const std::string& vertex_shader,
               const std::string& fragment_shader, BlendMode blend_mode = BlendMode::None);
        ...
        void set_uniform(const std::string& name, const Array& value)
        {
         ...
         */
        void draw_array(PrimitiveType primitive_type, size_t offset, size_t count,
                        bool indexed = false);
...
#if defined(NANOGUI_USE_OPENGL)
  #define NANOGUI_SHADER(name) NANOGUI_RESOURCE_STRING(name##_glsl)

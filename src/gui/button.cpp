...
namespace rl::gui {
    ...
    bool Button::mouse_button_event(const Vector2i& p, int button, bool down, int modifiers)
    {
        ...
        gui::ref<Button> self = this;

        if (m_enabled == 1 && ((button == GLFW_MOUSE_BUTTON_1 && !(m_flags & MenuButton)) ||
                               (button == GLFW_MOUSE_BUTTON_2 && (m_flags & MenuButton))))
        {
            ...
            if (down)
            {
                ...
                if (m_flags & PopupButton)
                {
                    ...
                    dynamic_cast<gui::PopupButton*>(this)->popup()->request_focus();
                }
    ...
    void Button::draw(NVGcontext* ctx)
    {
        ...
        if (m_icon)
        {
            ...
            {
                NVGpaint img_paint = nvgImagePattern(ctx, icon_pos.x(), icon_pos.y() - ih / 2, iw,
                                                     ih, 0, m_icon, m_enabled ? 0.5f : 0.25f);

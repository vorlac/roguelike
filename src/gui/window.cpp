...
namespace rl::gui {
    ...
    Vector2i Window::preferred_size(NVGcontext* ctx) const
    {
        ...
        return Vector2i(std::max(result.x(), (int)(bounds[2] - bounds[0] + 20)),
    ...
    void Window::perform_layout(NVGcontext* ctx)
    {
        ...
        else
        {
            ...
            m_button_panel->set_position(
                Vector2i(width() - (m_button_panel->preferred_size(ctx).x() + 5), 3));
    ...
    void Window::draw(NVGcontext* ctx)
    {
        ...
        /* Draw a drop shadow */
        NVGpaint shadow_paint = nvgBoxGradient(ctx, m_pos.x(), m_pos.y(), m_size.x(), m_size.y(),
        ...
        if (!m_title.empty())
        {
            ...
            NVGpaint header_paint = nvgLinearGradient(
                ctx, m_pos.x(), m_pos.y(), m_pos.x(), m_pos.y() + hh,
            ...
            nvgText(ctx, m_pos.x() + m_size.x() / 2, m_pos.y() + hh / 2 - 1, m_title.c_str(),
                    nullptr);
        }
    ...
    bool Window::mouse_drag_event(const Vector2i&, const Vector2i& rel, int button,
                                  int /* modifiers */)
    {

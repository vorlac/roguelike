...
#define NANOGUI_LAYOUT_OVERLOADS(Parent)                                                 \
    ...
    ::nanogui::Vector2i preferred_size(NVGcontext* ctx, const ::nanogui::Widget* widget) \
        const override                                                                   \
    {                                                                                    \
...
template <typename T>
struct type_caster<nanogui::ref<T>>
{
    using Value = nanogui::ref<T>;
    using Caster = make_caster<T>;
    constexpr static auto Name = Caster::Name;
    ...
    static handle from_cpp(const Value& value, rv_policy policy, cleanup_list* cleanup) noexcept
    {

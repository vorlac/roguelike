...
namespace rl::gui {
    ...
    namespace detail {
        ...
        template <typename T>
        using is_enoki_array_det = std::enable_if_t<T::IsEnoki>;
        template <typename T>
        using is_nanogui_array_det = std::enable_if_t<T::IsNanoGUI && !T::IsMatrix>;
        template <typename T>
        using is_nanogui_matrix_det = std::enable_if_t<T::IsNanoGUI && T::IsMatrix>;
    }
    ...
    template <typename T>
    constexpr bool
        is_nanogui_matrix_v = is_detected_v<detail::is_nanogui_matrix_det, std::decay_t<T>>;

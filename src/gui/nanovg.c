...
NVGcontext* nvgCreateInternal(NVGparams* params)
{
    ...
    // fontParams.width, fontParams.height, 0, NULL); if (ctx->fontImages[0] == 0) goto error;
    ctx->fontImageIdx = 0;
...
static NVGvertex* nvg__allocTempVerts(NVGcontext* ctx, int nverts)
{
    ...
    {
        NVGvertex* verts;
        int cverts = (nverts + 0xff) & ~0xff;  // Round up to prevent allocations when things change
                                               // just slightly.
...
void nvgFill(NVGcontext* ctx)
{
    ...
    ctx->params.renderFill(ctx->params.userPtr, &fillPaint, state->compositeOperation,
                           &state->scissor, ctx->fringeWidth, ctx->cache->bounds, ctx->cache->paths,
                           ctx->cache->npaths);

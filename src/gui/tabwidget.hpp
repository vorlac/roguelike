...
namespace rl::gui {
     ...
     */
    class TabWidgetBase : public Widget
    {
    public:
        ...
        int tab_count() const
        {
            return (int)m_tab_captions.size();
        };

        /// Return the ID of the tab at a given index
        int tab_id(int index) const
        {
            return m_tab_ids[index];
        };
        ...
        {
            return m_tab_ids.empty() ? -1 : tab_id(m_active_tab);
        }

        /// Set the ID of the currently active tab
        void set_selected_id(int id)
        {
            m_active_tab = tab_index(id);
            update_visibility();
        }
        ...
        const std::string& tab_caption(int id) const
        {
            return m_tab_captions[tab_index(id)];
        };

        /// Change the caption of the tab with the given ID
        void set_tab_caption(int id, const std::string& caption)
        {
            m_tab_captions[tab_index(id)] = caption;
        };

        /// Return whether tabs provide a close button
        bool tabs_closeable() const
        {
            return m_tabs_closeable;
        }
        ...
        bool tabs_draggable() const
        {
            return m_tabs_draggable;
        }

        void set_tabs_draggable(bool value)
        {
            m_tabs_draggable = value;
        }
        ...
        /// Callback that is used to notify a listener about tab changes (will be called with the
        /// tab ID)
        ...
        /// Set a callback that is used to notify a listener about tab changes (will be called with
        /// the tab ID)
        ...
        /// Callback that is used to notify a listener about tab close events (will be called with
        /// the tab ID)
        const std::function<void(int)>& close_callback() const
        {
            return m_close_callback;
        }

        /// Set a callback that is used to notify a listener about tab close events (will be called
        /// with the tab ID)
        void set_close_callback(const std::function<void(int)>& close_callback)
        {
            m_close_callback = close_callback;
        }

        /// Callback that is used to notify a listener about popup events (will be called with the
        /// tab ID)
        const std::function<Popup*(int, Screen*)>& popup_callback() const
        {
            return m_popup_callback;
        }

        /// Set a callback that is used to notify a listener about popup events (will be called with
        /// the tab ID)
        void set_popup_callback(const std::function<Popup*(int, Screen*)>& popup_callback)
        {
    ...
    protected:
        std::pair<int, bool> tab_at_position(const Vector2i& p, bool test_vertical = true) const;
     ...
     * \endrst
     */
    class TabWidget : public TabWidgetBase
    {
    public:
        /// Construct a new tab widget
        ...
        bool remove_children() const
        {
            return m_remove_children;
        }

        /// Remove child widgets when the associated tab is closed/removed?
        void set_remove_children(bool value)
        {
            m_remove_children = value;
        }

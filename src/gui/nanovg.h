...
#define NVG_PI 3.14159265358979323846264338327f
  ...
  #pragma warning(disable : 4201)  // nonstandard extension used : nameless struct/union
    ...
    struct NVGtextRow
    {
        ...
        float minx, maxx;   // Actual bounds of the row. Logical with and bounds can differ because
                            // of kerning and some parts over extending.
    };
    ...
    // Sets the composite operation with custom pixel arithmetic. The parameters should be one of
    // NVGblendFactor.
    ...
    // Render styles
    //
    // Fill and stroke render style can be either a solid color or a paint which is a gradient or a
    // pattern. Solid color is simply defined as a color value, different kinds of paints can be
    // created using nvgLinearGradient(), nvgBoxGradient(), nvgRadialGradient() and
    // nvgImagePattern().
    //
    ...
    //
    // Current coordinate system (transformation) can be saved and restored using nvgSave() and
    // nvgRestore().
    ...
    // Creates and returns a linear gradient. Parameters (sx,sy)-(ex,ey) specify the start and end
    // coordinates of the linear gradient, icol specifies the start color and ocol the end color.
    // The gradient is transformed by the current transform when it is passed to nvgFillPaint() or
    // nvgStrokePaint().
    ...
    // gradient is transformed by the current transform when it is passed to nvgFillPaint() or
    // nvgStrokePaint().
    NVGpaint nvgBoxGradient(NVGcontext* ctx, float x, float y, float w, float h, float r, float f,
                            NVGcolor icol, NVGcolor ocol);

    // Creates and returns a radial gradient. Parameters (cx,cy) specify the center, inr and outr
    // specify the inner and outer radius of the gradient, icol specifies the start color and ocol
    // the end color. The gradient is transformed by the current transform when it is passed to
    // nvgFillPaint() or nvgStrokePaint().
    NVGpaint nvgRadialGradient(NVGcontext* ctx, float cx, float cy, float inr, float outr,
                               NVGcolor icol, NVGcolor ocol);

    // Creates and returns an image patter. Parameters (ox,oy) specify the left-top location of the
    // image pattern, (ex,ey) the size of one image, angle rotation around the top-left corner,
    // image is handle to the image to render. The gradient is transformed by the current transform
    // when it is passed to nvgFillPaint() or nvgStrokePaint().
    NVGpaint nvgImagePattern(NVGcontext* ctx, float ox, float oy, float ex, float ey, float angle,
    ...
    int nvgTextGlyphPositions(NVGcontext* ctx, float x, float y, const char* string,
                              const char* end, NVGglyphPosition* positions, int maxPositions);
    ...
    // White space is stripped at the beginning of the rows, the text is split at word boundaries or
    // when new-line characters are encountered. Words longer than the max width are slit at nearest

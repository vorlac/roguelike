...
namespace nanort {
    ...
    class StackString
        : public StackContainer<
              std::basic_string<char, std::char_traits<char>, StackAllocator<char, stack_capacity>>,
              stack_capacity>
    {
    public:
        StackString()
    ...
    template <size_t stack_capacity>
    class StackWString
        : public StackContainer<std::basic_string<wchar_t, std::char_traits<wchar_t>,
                                                  StackAllocator<wchar_t, stack_capacity>>,
                                stack_capacity>
    {
    public:
        StackWString()
            : StackContainer<std::basic_string<wchar_t, std::char_traits<wchar_t>,
                                               StackAllocator<wchar_t, stack_capacity>>,
                             stack_capacity>()
        {
        }
    ...
    namespace {
        ...
        const float3* as_float3(const float* v)
        {
    ...
    struct Intersection
    {
        ...
        Intersection(float _t, float _u, float _v, unsigned int i)
            : t(_t)
            , u(_u)
            , v(_v)
            , faceID(i)
    ...
    namespace {
        ...
        typedef std::priority_queue<Intersection, std::vector<Intersection>, IsectComparator>
            IsectVector;
    ...
    public:
        ...
        bool Traverse(Intersection& isect, const float* vertices, const unsigned int* faces,
                      const Ray& ray, const BVHTraceOptions& options);

        // Multi-hit ray tracversal
        // Returns `maxIntersections` frontmost intersections
        bool MultiHitTraverse(StackVector<Intersection, 128>& isects, int maxIntersections,
                              const float* vertices, const unsigned int* faces, Ray& ray);
    ...
    private:
        ...
        unsigned int BuildShallowTree(std::vector<BVHNode>& outNodes, const float* vertices,
                                      const unsigned int* faces, unsigned int leftIdx,
                                      unsigned int rightIdx, int depth, int maxShallowDepth,
                                      float epsScale);
#endif

        // Builds BVH tree recursively.
        size_t BuildTree(BVHBuildStatistics& outStat, std::vector<BVHNode>& outNodes,
                         const float* vertices, const unsigned int* faces, unsigned int leftIdx,
...
#if defined(NANORT_IMPLEMENTATION) || defined(__INTELLISENSE__)
...
namespace nanort {
    ...
    inline void GetBoundingBoxOfTriangle(float3& bmin, float3& bmax, const float* vertices,
    ...
    void ContributeBinBuffer(BinBuffer* bins,  // [out]
                             const float3& sceneMin, const float3& sceneMax, const float* vertices,
                             const unsigned int* faces, unsigned int* indices, unsigned int leftIdx,
    ...
    inline float SAH(size_t ns1, float leftArea, size_t ns2, float rightArea, float invS,
                     float Taabb, float Ttri)
    {
    ...
    bool FindCutFromBinBuffer(float* cutPos,     // [out] xyz
                              int& minCostAxis,  // [out]
                              const BinBuffer* bins, const float3& bmin, const float3& bmax,
                              size_t numTriangles,
    ...
    void ComputeBoundingBoxOMP(float3& bmin, float3& bmax, const float* vertices,
                               ...
                               unsigned int leftIndex, unsigned int rightIndex, float epsScale)
    {
        const float kEPS = std::numeric_limits<float>::epsilon() * epsScale;
    ...
    void GetBoundingBox(float3& bmin, float3& bmax, std::vector<BBox>& bboxes,
                        unsigned int* indices, unsigned int leftIndex, unsigned int rightIndex,
                        float epsScale)
    {
        const float kEPS = std::numeric_limits<float>::epsilon() * epsScale;
...

        {
            ...
            {  // for each faces
                ...
                for (int k = 0; k < 3; k++)
                {  // xyz
                    float minval = bboxes[idx2].bmin[k] - kEPS;
                    float maxval = bboxes[idx2].bmax[k] + kEPS;
                    if (local_bmin[k] > minval)
                        local_bmin[k] = minval;
                    if (local_bmax[k] < maxval)
                        local_bmax[k] = maxval;
    ...
    unsigned int BVHAccel::BuildShallowTree(
        std::vector<BVHNode>& outNodes, const float* vertices, const unsigned int* faces,
        unsigned int leftIdx, unsigned int rightIdx, int depth, int maxShallowDepth, float epsScale)
    {
        ...
        float3 bmin, bmax;
        ComputeBoundingBox(bmin, bmax, vertices, faces, &indices_.at(0), leftIdx, rightIdx,
                           epsScale);
        ...
        else
        {
            ...
            leftChildIndex = BuildShallowTree(outNodes, vertices, faces, leftIdx, midIdx, depth + 1,
                                              maxShallowDepth, epsScale);

            rightChildIndex = BuildShallowTree(outNodes, vertices, faces, midIdx, rightIdx,
                                               depth + 1, maxShallowDepth, epsScale);

            if ((leftChildIndex != (unsigned int)(-1)) && (rightChildIndex != (unsigned int)(-1)))
            {
    ...
    inline size_t BVHAccel::BuildTree(BVHBuildStatistics& outStat, std::vector<BVHNode>& outNodes,
                                      ...
                                      float epsScale)
    {
        ...
        if (!bboxes_.empty())
        {
            GetBoundingBox(bmin, bmax, bboxes_, &indices_.at(0), leftIdx, rightIdx, epsScale);
        }
        else
        {
            ComputeBoundingBox(bmin, bmax, vertices, faces, &indices_.at(0), leftIdx, rightIdx,
        ...
        BinBuffer bins(options_.binSize);
        ContributeBinBuffer(&bins, bmin, bmax, vertices, faces, &indices_.at(0), leftIdx, rightIdx,
                            epsScale);
        FindCutFromBinBuffer(cutPos, minCutAxis, &bins, bmin, bmax, n, options_.costTaabb, epsScale);
    ...
    #pragma omp parallel for
        ...
        {
  #ifdef _OPENMP
            ComputeBoundingBoxOMP(bmin, bmax, vertices, faces, &indices_.at(0), 0, n, epsScale);
  #else
  ...
  // 3. Build tree
      ...
      #pragma omp parallel for
            for (int i = 0; i < (int)shallowNodeInfos_.size(); i++)
            {
                ...
                BuildTree(local_stats[i], local_nodes[i], vertices, faces, leftIdx, rightIdx,
                          options.shallowDepth, epsScale);
            }
            ...
            for (int i = 0; i < (int)local_nodes.size(); i++)
            {
                stats_.maxTreeDepth = std::max(stats_.maxTreeDepth, local_stats[i].maxTreeDepth);
    ...
    namespace {
        ...
        inline bool IntersectRayAABB(float& tminOut,  // [out]
                                     float& tmaxOut,  // [out]
                                     float maxT, float bmin[3], float bmax[3], const float3& rayOrg,
                                     const float3& rayInvDir, int rayDirSign[3])
        {
            float tmin, tmax;
        ...
        inline bool TestLeafNode(Intersection& isect,  // [inout]
                                 ...
                                 float epsScale, const BVHTraceOptions& traceOptions)
        {
            bool hit = false;
        ...
        inline bool MultiHitTestLeafNode(
            IsectVector& isects,  // [inout]
            int maxIntersections, const BVHNode& node, const std::vector<unsigned int>& indices,
            const float* vertices, const unsigned int* faces, const Ray& ray, float epsScale)
        {
            ...
            for (unsigned int i = 0; i < numTriangles; i++)
            {
                ...
                const float3 *v0, *v1, *v2;
                v0 = (const float3*)(vertices + 3 * (*(ff + 0)));
                v1 = (const float3*)(vertices + 3 * (*(ff + 1)));
                v2 = (const float3*)(vertices + 3 * (*(ff + 2)));
                                   ...
                                   const BVHTraceOptions& options)
    {
        ...
        while (nodeStackIndex >= 0)
        {
            ...
            bool hit = IntersectRayAABB(minT, maxT, hitT, node.bmin, node.bmax, rayOrg, rayInvDir,
                                        dirSign);
                                           ...
                                           const unsigned int* faces, Ray& ray)
    {
        ...
        while (nodeStackIndex >= 0)
        {
            ...
            {  // leaf node
                if (hit)
                {
                    if (MultiHitTestLeafNode(isectPQ, maxIntersections, node, indices_, vertices,
                                             faces, ray, epsScale_))
                    {

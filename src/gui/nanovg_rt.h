...
extern "C"
{
    ...
    void nvgClearBackgroundRT(NVGcontext* ctx, float r, float g, float b, float a);  // Clear
                                                                                     // background.
...
namespace {
    ...
    */
    ...
    public:
        ...
        TextureSampler(unsigned char* image, int width, int height, int components, Format format,
                       float gamma = 1.0f)
            : m_pow(gamma)
        {
        ...
        void Set(const unsigned char* image, int width, int height, int components, Format format,
                 float gamma = 1.0f)
        {
            m_width = width;
            m_height = height;
    ...
    inline int fasterfloor(const float x)
    {
        if (x >= 0)
            return (int)x;
    ...
    inline void FilterByteLerp(float* rgba, const unsigned char* image, int i00, int i10, int i01,
                               int i11, float dx, float dy, int stride, const PowGenerator& p)
    {
        float texel[4][4];
...
inline void FilterFloatLerp(float* rgba, const float* image, int i00, int i10, int i01, int i11,
                            float dx, float dy, int stride, const PowGenerator& p)
{
    float texel[4][4];
    ...
    if (stride == 4)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            rgba[i] = lerp(lerp(texel[0][i], texel[1][i], dx), lerp(texel[2][i], texel[3][i], dx),
                           dy);
        }
    }
    else
...
{
    unsigned char table[7][3] = { { 255, 0, 0 },    { 0, 0, 255 },   { 0, 255, 0 },
...
static int rtnvg__maxi(int a, int b)
{
    return a > b ? a : b;
}
...
static RTNVGtexture* rtnvg__allocTexture(RTNVGcontext* rt)
{
    ...
    if (tex == NULL)
    {
        ...
        {
            RTNVGtexture* textures;
            int ctextures = rtnvg__maxi(rt->ntextures + 1, 4) + rt->ctextures / 2;  // 1.5x
                                                                                    // Overallocate
...
static int rtnvg__createShader(RTNVGshader* shader, const char* name, const char* header,
...
static int rtnvg__renderCreate(void* uptr)
{
    ...
    {
        if (rtnvg__createShader(&rt->shader, "shader", shaderHeader, NULL, fillVertShader,
                                fillFragShader) == 0)
            return 0;
    }
...
static int rtnvg__renderCreateTexture(void* uptr, int type, int w, int h, int imageFlags,
...
static int rtnvg__renderUpdateTexture(void* uptr, int image, int x, int y, int w, int h,
                                      const unsigned char* data)
{
...
static float rtnvg__scissorMask(float scissorMat[12], float scissorExt[2], float scissorScale[2],
                                float x, float y)
{
...
static void rtnvg__shade(float color[4], RTNVGcontext* rt, RTNVGfragUniforms* frag, float x,
                         float y, float tu, float tv, int imageId)
{
    ...
    {  // grad fill
       // Calculate gradient color using box gradient
        ...
        float d = fclamp(
            (rtnvg__sdroundrect(pt, frag->extent, frag->radius) + frag->feather * 0.5f) /
                (float)frag->feather,
            0.0f, 1.0f);
...
static void rtnvg__fill(RTNVGcontext* rt, RTNVGcall* call)
{
    ...
    // render
    {
        ...
        if (faces.size() > 0)
        {
            ...
            bool ret = accel.Build(&vertices.at(0), &faces.at(0), (unsigned int)faces.size() / 3,
                                   options);
            assert(ret);
            (void)ret;
            ...
            for (int y = bound[1]; y < bound[3]; y++)
            {
                ...
                for (int x = bound[0]; x < bound[2]; x++)
                {
                    ...
                    {
                        float col[4];
                        RTNVGfragUniforms* frag = nvg__fragUniformPtr(
                            rt, call->uniformOffset + rt->fragSize);
                            ...
                            if (hit)
                            {
                                ...
                                for (size_t i = 0; i < isects->size(); i++)
                                {
                                    ...
                                    float tv = (1.0f - U - V) * texcoords[2 * f0 + 1] +
                                               ...
                                               V * texcoords[2 * f2 + 1];
                                    RTNVGfragUniforms* frag = nvg__fragUniformPtr(
                                        rt, call->uniformOffset);
                                    rtnvg__shade(fragCol, rt, frag, ray.org[0], ray.org[1], tu, tv,
                                                 call->image);
...
static int rtnvg__allocFragUniforms(RTNVGcontext* rt, int n)
{
    ...
    {
        unsigned char* uniforms;
        int cuniforms = rtnvg__maxi(rt->nuniforms + n, 128) + rt->cuniforms / 2;  // 1.5x
                                                                                  // Overallocate
...
static void rtnvg__renderFill(void* uptr, NVGpaint* paint,
                              ...
                              float fringe, const float* bounds, const NVGpath* paths, int npaths)
{
    ...
    if (call->type == RTNVG_FILL)
    {
        ...
        // Fill shader
        rtnvg__convertPaint(rt, nvg__fragUniformPtr(rt, call->uniformOffset + rt->fragSize), paint,
    ...
    else
    {
        ...
        if (call->uniformOffset == -1)
            goto error;
        // Fill shader
        rtnvg__convertPaint(rt, nvg__fragUniformPtr(rt, call->uniformOffset), paint, scissor,
                            fringe, fringe, -1.0f);
    }
...
inline int nvglCreateImageFromHandle(NVGcontext* ctx, unsigned int textureId, int w, int h,
                                     int imageFlags)
{
    RTNVGcontext* rt = (RTNVGcontext*)nvgInternalParams(ctx)->userPtr;
    RTNVGtexture* tex = rtnvg__allocTexture(rt);

...
namespace rl::gui {
    ...
    TabWidgetBase::TabWidgetBase(Widget* parent, const std::string& font)
        : Widget(parent)
        , m_font(font)
        , m_background_color(Color(0.f, 0.f))
    {
    ...
    void TabWidgetBase::perform_layout(NVGcontext* ctx)
    {
        ...
        m_close_width = nvgTextBounds(ctx, 0, 0, utf8(FA_TIMES_CIRCLE).data(), nullptr, unused);
    }
    ...
    void TabWidgetBase::draw(NVGcontext* ctx)
    {
        if (m_tab_offsets.size() != m_tab_captions.size() + 1)
            throw std::runtime_error(
                "Must run TabWidget::perform_layout() after adding/removing tabs!");
        ...
        {
            int x_pos = m_pos.x() + m_tab_offsets[i], y_pos = m_pos.y(),
                width = m_tab_offsets[i + 1] - m_tab_offsets[i];

            if (i == (size_t)m_active_tab)
            {
                nvgBeginPath(ctx);
                nvgRoundedRect(ctx, x_pos + 0.5f, y_pos + 1.5f, width, tab_height + 4,
                               m_theme->m_button_corner_radius);
                nvgStrokeColor(ctx, m_theme->m_border_light);
                nvgStroke(ctx);

                nvgBeginPath(ctx);
                nvgRoundedRect(ctx, x_pos + 0.5f, y_pos + 0.5f, width, tab_height + 4,
                               m_theme->m_button_corner_radius);
            ...
            if (x >= m_tab_offsets[i] && x < m_tab_offsets[i + 1])
            {
                ...
                return { (int)i,
                         m_tabs_closeable &&
                             ...
                             p.y() - m_pos.y() <=
                                 tab_height - m_theme->m_tab_button_vertical_padding };
            }
        }
        return { -1, false };
    }

    bool TabWidgetBase::mouse_button_event(const Vector2i& p, int button, bool down, int modifiers)
    {
        int index;
        bool close;
        ...
        if (m_popup)
        {
            m_popup->mouse_button_event(p - m_pos + absolute_position() -
                                            m_popup->absolute_position() + m_popup->position(),
    ...
    bool TabWidgetBase::mouse_motion_event(const Vector2i& p, const Vector2i& rel, int button,
                                           int modifiers)
    {
        ...
        if (m_tab_drag_index != -1)
        {
            ...
            {
                int i0 = std::min(m_tab_drag_index, index), i1 = std::max(m_tab_drag_index, index);
    ...
    {
        Vector2i base_size = TabWidgetBase::preferred_size(ctx), content_size = Vector2i(0);
        for (Widget* child : m_children)
            content_size = max(content_size, child->preferred_size(ctx));

        return Vector2i(std::max(base_size.x(), content_size.x() + 2 * m_padding),

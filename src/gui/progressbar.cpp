...
namespace rl::gui {
    ...
    void ProgressBar::draw(NVGcontext* ctx)
    {
        ...
        NVGpaint paint = nvgBoxGradient(ctx, m_pos.x() + 1, m_pos.y() + 1, m_size.x() - 2,
        ...
        paint = nvgBoxGradient(ctx, m_pos.x(), m_pos.y(), bar_pos + 1.5f, m_size.y() - 1, 3, 4,
                               Color(220, 100), Color(128, 100));

        nvgBeginPath(ctx);
        nvgRoundedRect(ctx, m_pos.x() + 1, m_pos.y() + 1, bar_pos, m_size.y() - 2, 3);
        nvgFillPaint(ctx, paint);
        nvgFill(ctx);
    }

...
#define NANOGUI_VERSION                                            \
    (NANOGUI_TOSTRING(NANOGUI_VERSION_MAJOR) "." NANOGUI_TOSTRING( \
        NANOGUI_VERSION_MINOR) "." NANOGUI_TOSTRING(NANOGUI_VERSION_PATCH))
...
// #define NANOGUI_SHOW_WIDGET_BOUNDS 1
   ...
   *
   * The macro ``namespace rl::gui {`` will expand to ``namespace
...
namespace rl::gui {
    ...
    /// Cursor shapes available to use in GLFW.  Shape of actual cursor determined by Operating
    /// System.
    enum class Cursor {
        ...
        CursorCount  ///< Not a cursor --- should always be last: enables a loop over the cursor
                     ///< types.
    };
     ...
     */
    extern void mainloop(float refresh = -1.f);
     ...
     */
    extern std::pair<bool, bool> test_10bit_edr_support();
    ...
    extern std::vector<std::pair<int, std::string>> load_image_directory(NVGcontext* ctx,

...
//        render glyphs to one-channel bitmaps with antialiasing (box filter)
  ...
  // #define your own (u)stbtt_int8/16/32
  // before including to override this
  #ifndef stbtt_uint8
...
extern "C"
{
    ...
    STBTT_DEF int stbtt_BakeFontBitmap(const unsigned char* data, int offset,  // font location (use
                                                                               // offset=0 for plain
                                                                               // .ttf)
    ...
    STBTT_DEF void stbtt_GetBakedQuad(stbtt_bakedchar* chardata, int pw, int ph,  // same data as
                                                                                  // above
                                      int char_index,            // character to display
                                      float* xpos, float* ypos,  // pointers to current position in
                                                                 // screen pixel space
                                      stbtt_aligned_quad* q,     // output: quad to draw
                                      int opengl_fillrule);  // true if opengl fill rule; false if
                                                             // DX9 or earlier
  ...
  #define STBTT_POINT_SIZE(x) (-(x))
    ...
    STBTT_DEF void stbtt_GetPackedQuad(stbtt_packedchar* chardata, int pw, int ph,  // same data as
                                                                                    // above
                                       int char_index,            // character to display
                                       float* xpos, float* ypos,  // pointers to current position in
                                                                  // screen pixel space
    ...
    STBTT_DEF int stbtt_PackFontRangesGatherRects(stbtt_pack_context* spc, stbtt_fontinfo* info,
    ...
    STBTT_DEF void stbtt_Rasterize(stbtt__bitmap* result,     // 1-channel bitmap to draw into
                                   ...
                                   float shift_x, float shift_y,  // translation applied to input
                                                                  // vertices
...
#if defined(STB_TRUETYPE_IMPLEMENTATION) || defined(__INTELLISENSE__)
                                             ...
                                             float start_point, void* userdata)
{
    ...
    z->x = STBTT_ifloor(STBTT_FIX * e->x0 + z->dx * (start_point - e->y0));  // use z->dx so when we
                                       ...
                                       float y0, float x1, float y1)
{
    ...
    else
    {
        ...
        scanline[x] += e->direction * (y1 - y0) * (1 - ((x0 - x) + (x1 - x)) / 2);  // coverage = 1
                                                                                    // - average x
                                                                                    // position
    }
}
        ...
        else
        {
            ...
            if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len)
            {
                ...
                if ((int)x_top == (int)x_bottom)
                {
                    ...
                    scanline_fill[x] += e->direction * height;  // everything right of this pixel is
                                                                // filled
                }
                else
                {
...
STBTT_DEF void stbtt_GetBakedQuad(stbtt_bakedchar* chardata, int pw, int ph, int char_index,
...
STBTT_DEF int stbtt_PackFontRange(stbtt_pack_context* spc, unsigned char* fontdata, int font_index,
...
static stbtt_int32 stbtt__CompareUTF8toUTF16_bigendian_prefix(
    const stbtt_uint8* s1, stbtt_int32 len1, const stbtt_uint8* s2, stbtt_int32 len2)
{
    stbtt_int32 i = 0;
...
{
    return len1 == stbtt__CompareUTF8toUTF16_bigendian_prefix((const stbtt_uint8*)s1, len1,
...
STBTT_DEF int stbtt_FindMatchingFont(const unsigned char* font_collection, const char* name_utf8,
                                     stbtt_int32 flags)
{
    stbtt_int32 i;
    for (i = 0;; ++i)

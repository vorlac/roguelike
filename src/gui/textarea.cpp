...
namespace rl::gui {
    ...
    TextArea::TextArea(Widget* parent)
        : Widget(parent)
        , m_foreground_color(Color(0, 0))
        , m_background_color(Color(0, 0))
        , m_selection_color(.5f, 1.f)
        , m_font("sans")
        , m_offset(0)
        , m_max_size(0)
        , m_padding(0)
                ...
                m_selection_start != -1 && m_selection_end != -1)
            {
                ...
                for (int i = start.x(); i <= end.x(); ++i)
                {
                    ...
                    int nglyphs = nvgTextGlyphPositions(ctx, block.offset.x(), block.offset.y(),
                                                        block.text.c_str(), nullptr, glyphs,
                                                        max_glyphs);
    ...
    void TextArea::draw(NVGcontext* ctx)
    {
        ...
        std::vector<Block>::iterator start_it = m_blocks.begin(), end_it = m_blocks.end();
        if (vscroll)
        {
            int window_offset = -position().y(), window_size = vscroll->size().y();

            start_it = std::lower_bound(m_blocks.begin(), m_blocks.end(), window_offset,
                                        [&](const Block& block, int value) {
                                            return block.offset.y() + font_size() < value;
                                        });

            end_it = std::upper_bound(m_blocks.begin(), m_blocks.end(), window_offset + window_size,
        ...
        if (m_selection_end != Vector2i(-1) && m_selection_end != Vector2i(-1))
        {
            ...
            if (selection_end.y() == selection_start.y())
            {
                nvgRect(ctx, selection_start.x(), selection_start.y(),
                        selection_end.x() - selection_start.x(), font_size());
            }
            else
            {
                nvgRect(ctx, selection_start.x(), selection_start.y(),
    ...
    bool TextArea::mouse_button_event(const Vector2i& p, int button, bool down, int /* modifiers */)
    {
        ...
        {
            m_selection_start = m_selection_end = position_to_block(p - m_pos - m_padding);
            request_focus();
            return true;
        }
    ...
    bool TextArea::mouse_drag_event(const Vector2i& p, const Vector2i& /* rel */, int /* button */,
                                    int /* modifiers */)
    {
    ...
    {
        NVGcontext* ctx = screen()->nvg_context();
        auto it = std::lower_bound(m_blocks.begin(), m_blocks.end(), pos.y(),
                                   [&](const Block& block, int value) {
                                       return block.offset.y() + font_size() < value;
                                   });

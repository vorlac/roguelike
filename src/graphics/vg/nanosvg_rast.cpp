  ...
  #define NSVG_PI (3.14159265358979323846264338327f)
    ...
    static void nsvg__prepareStroke(NSVGrasterizer* r, float miterLimit, int lineJoin)
    {
        ...
        for (j = 0; j < r->npoints; j++)
        {
            ...
            if (p1->flags & NSVG_PT_CORNER)
            {
                if ((dmr2 * miterLimit * miterLimit) < 1.0f || lineJoin == NSVG_JOIN_BEVEL ||
                    lineJoin == NSVG_JOIN_ROUND)
                {
    ...
    static void nsvg__flattenShapeStroke(NSVGrasterizer* r, const NSVGshape* shape, float sx,
                                         float sy)
    {
        int i, j, closed;
        NSVGpath* path;
    ...
    static unsigned int nsvg__RGBA(unsigned char r, unsigned char g, unsigned char b,
                                   unsigned char a)
    {
    ...
    static void nsvg__scanlineSolid(unsigned char* dst, int count, unsigned char* cover, int x,
                                    int y, float tx, float ty, float sx, float sy,
    ...
    static void nsvg__rasterizeSortedEdges(NSVGrasterizer* r, float tx, float ty, float sx,
                                           float sy, NSVGcachedPaint* cache, char fillRule)
    {
    ...
    e->x0,e->y0, e->x1,e->y1);
        ...
        for (i = 0; i < r->npoints; i++) {
            if (i+1 < r->npoints)
                fprintf(fp ,"<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" style=\"stroke:#f00;\"
    />", r->points[i].x, r->points[i].y, r->points[i+1].x, r->points[i+1].y); fprintf(fp ,"<circle
    cx=\"%f\" cy=\"%f\" r=\"1\" style=\"fill:%s;\" />", r->points[i].x, r->points[i].y,
    r->points[i].flags == 0 ?
    "#f00" : "#0f0");
        }

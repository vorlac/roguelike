...
namespace rl::vg {
    ...
    // can be created using nvgLinearGradient(), nvgBoxGradient(), nvgRadialGradient() and
    // nvgImagePattern().
    //
    ...
    int nvgCreateImageAlpha(NVGcontext* ctx, int w, int h, int imageFlags,
    ...
    // Paths
    //
    // Drawing a new shape starts with nvgBeginPath(), it clears all the currently defined
    ...
    // path you can call nvgPathWinding(). This is useful especially for the common shapes,
    // which are drawn CCW.
    //
    // Finally you can fill the path using current fill style by calling nvgFill(), and stroke
    // it with current stroke style by calling nvgStroke().
    //
    ...
    void nvgBarc(NVGcontext* ctx, float cx, float cy, float r, float a0, float a1, int dir,
                 int join);
    ...
    int nvgCreateFontAtIndex(NVGcontext* ctx, const char* name, const char* filename,
                             const int fontIndex);
    ...
    // fontIndex specifies which font face to load from a .ttf/.ttc file.
    int nvgCreateFontMemAtIndex(NVGcontext* ctx, const char* name, unsigned char* data, int ndata,

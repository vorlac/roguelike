...
namespace rl::inline utils {
    namespace detail {
        template <typename H>
        void to_arg_vec_impl(std::vector<std::string>& s, H&& h) {
            s.push_back(typeid(decltype(h)).name());
        }

        template <typename H, typename... T>
        void to_arg_vec_impl(std::vector<std::string>& s, H&& h, T&&... t) {
            s.push_back(typeid(decltype(h)).name());
            to_arg_vec_impl(s, std::forward<T>(t)...);
        }

        template <typename... TupleTypes, std::size_t... Idx>
        std::vector<std::string> to_arg_vec(const std::tuple<TupleTypes...>& tup,
        ...
        auto arg_vec_to_tuple(const std::vector<std::string>& v, std::index_sequence<Indexes...>) {
            return std::make_tuple(v[Indexes]...);
        }

        template <std::size_t N>
        auto arg_vec_to_tuple(const std::vector<std::string>& v) {
    ...
    struct function_traits<TRet (TClass::*)(TArgs...)> {
        ...
        static inline const std::string_view class_name{ typeid(class_type).name() };
    ...
    struct function_traits
        ...
        static inline const std::string_view class_name{};
    };

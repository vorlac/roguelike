...
namespace rl::inline constraint {
    ...
    template <typename... T>
    concept integer = ((
                           unsigned_integer<std::type_identity_t<T>> || signed_integer<std::type_identity_t<T>>)
                       && ...);

    template <typename... T>
    concept numeric = ((
                           floating_point<std::type_identity_t<T>> || integer<std::type_identity_t<T>>)
                       && ...);
    ...
    concept higher_precision = (floating_point<L, R>
                                && greater_than<std::numeric_limits<L>::max(),

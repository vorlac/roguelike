...
namespace rl::gui {
    ...
    static GLuint compile_gl_shader(GLenum type, const std::string& name,
                                    const std::string& shader_string)
    {
        ...
        if (status != GL_TRUE)
        {
            ...
            std::string msg = std::string("compile_gl_shader(): unable to compile ") + type_str +
                              " \"" + name + "\":\n\n" + error_shader;
    ...
    Shader::Shader(RenderPass* render_pass, const std::string& name,
        ...
        , m_shader_handle(0)
    {
        ...
        for (int i = 0; i < attribute_count; ++i)
        {
            ...
            GLenum type = 0;
            GLint size = 0;
            CHK(glGetActiveAttrib(m_shader_handle, i, sizeof(attr_name), nullptr, &size, &type,
                                  attr_name));
        ...
        for (int i = 0; i < uniform_count; ++i)
        {
            ...
            GLenum type = 0;
            GLint size = 0;
            CHK(glGetActiveUniform(m_shader_handle, i, sizeof(uniform_name), nullptr, &size, &type,
                                   uniform_name));
    ...
    void Shader::set_buffer(const std::string& name, VariableType dtype, size_t ndim,

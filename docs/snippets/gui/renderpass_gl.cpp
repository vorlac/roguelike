...
namespace rl::gui {
    ...
    RenderPass::RenderPass(const std::vector<Object*>& color_targets, Object* depth_target,
                           Object* stencil_target, Object* blit_target, bool clear)
        : m_targets(color_targets.size() + 2)
        , m_targets_ref(color_targets.size() + 2)
        , m_clear_color(color_targets.size())
        , m_clear(clear)
        , m_clear_stencil(0)
        , m_clear_depth(1.f)
        , m_viewport_offset(0)
        ...
        , m_framebuffer_handle(0)
    {
        ...
        bool has_texture = false, has_screen = false;
    ...
    void RenderPass::begin()
    {
        ...
        if (m_clear)
        {
            ...
            if (m_targets[2])
            {
                CHK(glClearColor(m_clear_color[0].r(), m_clear_color[0].g(), m_clear_color[0].b(),
                                 m_clear_color[0].w()));
    ...
    void RenderPass::end()
    {
        ...
        CHK(glViewport(m_viewport_backup[0], m_viewport_backup[1], m_viewport_backup[2],
                       m_viewport_backup[3]));
        CHK(glScissor(m_scissor_backup[0], m_scissor_backup[1], m_scissor_backup[2],
                      m_scissor_backup[3]));
    ...
    void RenderPass::blit_to(const Vector2i& src_offset, const Vector2i& src_size, Object* dst,
                             const Vector2i& dst_offset)
    {
        ...
        Vector2i src_end = src_offset + src_size, dst_end = dst_offset + src_size;

        CHK(glBlitFramebuffer((GLsizei)src_offset.x(), (GLsizei)src_offset.y(),
                              (GLsizei)src_end.x(), (GLsizei)src_end.y(), (GLsizei)dst_offset.x(),
                              (GLsizei)dst_offset.y(), (GLsizei)dst_end.x(), (GLsizei)dst_end.y(),
                              what, GL_NEAREST));

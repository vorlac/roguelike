...
namespace rl::gui {
    ...
    void PopupButton::draw(NVGcontext* ctx)
    {
        ...
        if (m_chevron_icon)
        {
            ...
            nvgFontSize(
                ctx, (m_font_size < 0 ? m_theme->m_button_font_size : m_font_size) * icon_scale());
    ...
    void PopupButton::perform_layout(NVGcontext* ctx)
    {
        ...
        {
            m_popup->set_position(absolute_position() + Vector2i(width() + anchor_size + 1,
                                                                 m_size.y() / 2 - anchor_size));
        }
    }

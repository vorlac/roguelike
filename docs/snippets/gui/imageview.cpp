...
namespace rl::gui {
        ...
        : Canvas(parent, 1, false, false, false)
    {
        ...
        m_image_shader = new Shader(render_pass(),
                                    /* An identifying name */
                                    "a_simple_shader", NANOGUI_SHADER(imageview_vertex),
    ...
    void ImageView::center()
    {
        if (!m_image)
            return;
        m_offset = Vector2i(.5f * (Vector2f(m_size) * screen()->pixel_ratio() -
                                   Vector2f(m_image->size()) * scale()));
    }
    ...
    bool ImageView::scroll_event(const Vector2i& p, const Vector2f& rel)
    {
        ...
        m_scale = std::max(
            m_scale,
            std::min(0.f,
                     std::log2(40.f / std::max(m_image->size().x(), m_image->size().y())) * 5.f));
    ...
    void ImageView::draw(NVGcontext* ctx)
    {
        ...
        if (m_draw_image_border)
        {
            ...
            nvgRect(ctx, m_pos.x() - .5f + top_left.x(), m_pos.y() - .5f + top_left.y(),
        ...
        if (scale() > 100 && m_pixel_callback)
        {
            ...
            char text_buf[80], *text[4] = { text_buf, text_buf + 20, text_buf + 40, text_buf + 60 };
    ...
    void ImageView::draw_contents()
    {
        ...
        Matrix4f matrix_background = Matrix4f::scale(
            Vector3f(m_image->size().x() * scale / 20.f, m_image->size().y() * scale / 20.f, 1.f));

        Matrix4f matrix_image = Matrix4f::ortho(0.f, viewport_size.x(), viewport_size.y(), 0.f,
                                                -1.f, 1.f) *

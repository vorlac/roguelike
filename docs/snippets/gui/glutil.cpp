...
namespace rl::gui {
    ...
    bool GLShader::initFromFiles(const std::string& name, const std::string& vertex_fname,
                                 ...
                                 const std::string& geometry_fname)
    {
        ...
        return init(name, file_to_string(vertex_fname), file_to_string(fragment_fname),
                    file_to_string(geometry_fname));
    }

    bool GLShader::init(const std::string& name, const std::string& vertex_str,
                        const std::string& fragment_str, const std::string& geometry_str)
    {
        ...
        mName = name;
        mVertexShader = createShader_helper(GL_VERTEX_SHADER, name, defines, vertex_str);
        mGeometryShader = createShader_helper(GL_GEOMETRY_SHADER, name, defines, geometry_str);
        mFragmentShader = createShader_helper(GL_FRAGMENT_SHADER, name, defines, fragment_str);
    ...
    void GLShader::setUniform(const std::string& name, const GLUniformBuffer& buf, bool warn)
    {
        ...
        {
            if (warn)
                std::cerr << mName << ": warning: did not find uniform buffer " << name
                          << std::endl;
            return;
        }
    ...
    void GLShader::uploadAttrib(const std::string& name, size_t size, int dim, uint32_t compSize,
                                GLuint glType, bool integral, const void* data, int version)
    {
        int attribID = 0;
        if (name != "indices")
        {
    ...
    void GLShader::shareAttrib(const GLShader& otherShader, const std::string& name,
                               const std::string& _as)
    {
        ...
        if (it == otherShader.mBufferObjects.end())
            throw std::runtime_error(
                "shareAttribute(" + otherShader.mName + ", " + name + "): attribute not found!");
        ...
        if (name != "indices")
        {
            ...
            glVertexAttribPointer(attribID, buffer.dim, buffer.glType,
                                  buffer.compSize == 1 ? GL_TRUE : GL_FALSE, 0, 0);
        }
        else
        {
    ...
    void GLFramebuffer::init(const Eigen::Vector2i& size, int nSamples)
    {
        ...
        if (nSamples <= 1)
            ...
            glRenderbufferStorageMultisample(GL_RENDERBUFFER, nSamples, GL_RGBA8, size.x(),
                                             size.y());
        ...
        if (nSamples <= 1)
            ...
            glRenderbufferStorageMultisample(GL_RENDERBUFFER, nSamples, GL_DEPTH24_STENCIL8,
                                             size.x(), size.y());

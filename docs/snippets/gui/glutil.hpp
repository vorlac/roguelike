...
namespace rl::gui {
    ...
    namespace detail {
        ...
        template <>
        struct type_traits<half_float::half>
        {
            enum {
                type = GL_HALF_FLOAT,
                integral = 0
    ...
    {
        template <typename T>
        friend struct detail::serialization_helper;
    ...
    public:
        ...
        bool init(const std::string& name, const std::string& vertex_str,
                  const std::string& fragment_str, const std::string& geometry_str = "");
         ...
         *     shader will be used.
         */
        bool initFromFiles(const std::string& name, const std::string& vertex_fname,
        ...
        void uploadAttrib(const std::string& name, const Matrix& M, int version = -1)
        {
            ...
            uploadAttrib(name, (uint32_t)M.size(), (int)M.rows(), compSize, glType, integral,
                         M.data(), version);
        }
        ...
        void downloadAttrib(const std::string& name, Matrix& M)
        {
            ...
            if (it == mBufferObjects.end())
                throw std::runtime_error(
                    "downloadAttrib(" + mName + ", " + name + ") : buffer not found!");
        ...
        /// Create a symbolic link to an attribute of another GLShader. This avoids duplicating
        /// unnecessary data
        void shareAttrib(const GLShader& otherShader, const std::string& name,
                         const std::string& as = "");
        ...
        void setUniform(const std::string& name, const Eigen::Transform<T, 3, 3>& affine,
                        bool warn = true)
        {
            glUniformMatrix4fv(uniform(name, warn), 1, GL_FALSE,
                               affine.template cast<float>().data());
        }
        ...
        void setUniform(const std::string& name, const Eigen::Transform<T, 2, 2>& affine,
                        bool warn = true)
        {
            glUniformMatrix3fv(uniform(name, warn), 1, GL_FALSE,
                               affine.template cast<float>().data());
        }
        ...
        template <typename T,
                  typename std::enable_if<detail::type_traits<T>::integral == 1, int>::type = 0>
        ...
        template <typename T,
                  typename std::enable_if<detail::type_traits<T>::integral == 0, int>::type = 0>
        void setUniform(const std::string& name, T value, bool warn = true)
        {
        ...
        {
            size_t size = 0;
            for (const auto& buf : mBufferObjects)
         ...
         *    :member:`mBufferObjects <GLShader::mBufferObjects>`.
         *
         ...
         *    :func:`attribVersion <GLShader::attribVersion>`.  If you want
         *    to share data between :class:`GLShader <GLShader>` objects,
         *    call :func:`shareAttrib <GLShader::shareAttrib>`.
         *
         ...
         *    buffer's :member:`Buffer::id <GLShader::Buffer::id>` is the
    ...
    public:
        /* Low-level API */
        void uploadAttrib(const std::string& name, size_t size, int dim, uint32_t compSize,
                          GLuint glType, bool integral, const void* data, int version = -1);
        void downloadAttrib(const std::string& name, size_t size, int dim, uint32_t compSize,
                            GLuint glType, void* data);
    ...
    public:
        ...
        template <typename Derived,
                  typename std::enable_if<Derived::IsVectorAtCompileTime, int>::type = 0>
    ...
    public:
        ...
        /// Quick and dirty method to write a TGA (32bpp RGBA) file of the framebuffer contents for
        /// debugging
     ...
     * mouse callbacks defined in \ref Widget.  The Arcball operates by
     ...
     *
     * A very simple \ref Screen derived class to illustrate usage:
     *
     * \rst
     * .. code-block:: cpp
     *
     *    class ArcballScreen : public Screen {
     *    public:
     *        // Creating a 400x400 window
     *        ArcballScreen() : Screen({400, 400}, "ArcballDemo") {
     ...
     *        }
     *
     *        virtual bool mouseMotionEvent(const Vector2i &p, const Vector2i &rel, int button, int
     * modifiers) override { if (button == GLFW_MOUSE_BUTTON_1) { mArcball.motion(p);// Note 2
     *                return true;
     *            }
     *            return false;
     *        }
     ...
     *     :func:`Arcball::setSize <Arcball::setSize>`, this does **not**
     ...
     *     :func:`Widget::mouseButtonEvent <Widget::mouseButtonEvent>`
     *     and :func:`Widget::mouseMotionEvent <Widget::mouseMotionEvent>`
     ...
     *     ``p`` before calling :func:`Arcball::button <Arcball::button>`
     *     or :func:`Arcball::motion <Arcball::motion>`.  For example, if
    ...
    struct Arcball
    {
         ...
         * .. note::
         *
         *    Make sure to call :func:`Arcball::setSize <Arcball::setSize>`
         *    after construction.
         * \endrst
         *
         * \param speedFactor
         ...
         */
        Eigen::Quaternionf& state()
        {
            return mQuat;
        }

        /// ``const`` version of \ref Arcball::state.
        const Eigen::Quaternionf& state() const
        {
            return mQuat;
        }
        ...
        Eigen::Quaternionf activeState() const
        {
            return mIncr * mQuat;
        }
         ...
         * have a callback that created a \ref MessageDialog which will now
         * be in focus.
         */
        void interrupt()
        {
            button(Eigen::Vector2i::Zero(), false);
        }
     ...
     */
    extern Vector3f project(const Vector3f& obj, const Matrix4f& model, const Matrix4f& proj,
     ...
     */
    extern Vector3f unproject(const Vector3f& win, const Matrix4f& model, const Matrix4f& proj,
                              const Vector2i& viewportSize);
     ...
     * \endrst
     */
    extern Matrix4f lookAt(const Vector3f& origin, const Vector3f& target, const Vector3f& up);
     ...
     *     The far plane.
     */
    extern Matrix4f ortho(float left, float right, float bottom, float top, float nearVal,
                          float farVal);
     ...
     *     The far plane.
     */
    extern Matrix4f frustum(float left, float right, float bottom, float top, float nearVal,
                            float farVal);
    /**

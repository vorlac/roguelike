...
namespace rl::gui {
    ...
    struct Array
    {
        constexpr static bool IsNanoGUI = true;
        ...
        template <
            typename T,
            std::enable_if_t<T::Size == Size && std::is_same_v<typename T::Value, Value>, int> = 0>
        Array(const T& a)
        {
    ...
    {
        return Array<Value, 3>(a.y() * b.z() - a.z() * b.y(), a.z() * b.x() - a.x() * b.z(),
                               a.x() * b.y() - a.y() * b.x());
    }
    ...
    public:
        ...
        Color(int r, int g, int b, int a)
            : Color(Vector4f((float)r, (float)g, (float)b, (float)a) / 255.f)
        {
        }
    ...
    struct Matrix
    {
        ...
        {
            Value s = std::sin(angle), c = std::cos(angle), t = 1 - c;
            ...
            Value tmp1 = axis.x() * axis.y() * t, tmp2 = axis.z() * s;
        ...
        {
            Value recip = 1 / (near_ - far_), c = 1 / std::tan(.5f * fov);

            Matrix trafo = Matrix::scale(
                Array<Value, Size>(c / aspect, c, (near_ + far_) * recip, 0.f));
        ...
        template <size_t S = Size, std::enable_if_t<S == 4, int> = 0>
        static Matrix ortho(Value left, Value right, Value bottom, Value top, Value near_,
                            Value far_)
        {
            Value rl = 1 / (right - left), tb = 1 / (top - bottom), fn = 1 / (far_ - near_);
        ...
        template <size_t S = Size, std::enable_if_t<S == 4, int> = 0>
        static Matrix look_at(const Array<Value, 3>& origin, const Array<Value, 3>& target,

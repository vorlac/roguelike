...
static void DrawPoints(SDL_Renderer* renderer)
{
    ...
    for (i = 0; i < num_objects * 4; ++i)
    {
        ...
        SDL_SetRenderDrawColor(renderer, 255, (Uint8)current_color, (Uint8)current_color,
                               (Uint8)current_alpha);
...
static void DrawLines(SDL_Renderer* renderer)
{
    ...
    for (i = 0; i < num_lines; ++i)
    {
        if (i == -1)
        {
            ...
            SDL_RenderLine(renderer, 0.0f, (float)(viewport.h / 2), (float)(viewport.w - 1),
                           (float)(viewport.h / 2));
            SDL_RenderLine(renderer, (float)(viewport.w / 2), 0.0f, (float)(viewport.w / 2),
                           (float)(viewport.h - 1));
        }
        else
        {
            ...
            case SDL_EVENT_KEY_DOWN:
                switch (event.key.keysym.sym)
                {
                    case 'l':
                        ...
                        {
                            add_line((float)(rand() % 640), (float)(rand() % 480),
                                     (float)(rand() % 640), (float)(rand() % 480));
                        }
                        break;
                    case 'r':
                        if (event.key.keysym.mod & SDL_KMOD_SHIFT)
                        ...
                        {
                            add_rect((float)(rand() % 640), (float)(rand() % 480),
                                     (float)(rand() % 640), (float)(rand() % 480));
                        }
                        break;
                }
...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc;)
    {
        ...
        if (consumed < 0)
        {
            const static char* options[] = { "[--blend none|blend|add|mod|mul]", "[--cyclecolor]",
                                             "[--cyclealpha]", "[count]", NULL };
